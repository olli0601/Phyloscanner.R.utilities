...
#' @param regex.tip Regular expression that identifies reads from query individuals
...
phsc.combine.phyloscanner.output<- function(in.dir, save.file=NA, postfix.trees='trees.rda', postfix.trmstats='trmStats.rda', regex.tip="^(.*)_read_([0-9]+)_count_([0-9]+)$", regex.ind="^[0-9]+_[0-9]_[0-9]+")
{
	#	read trees
	cat('\nread trees')
	...
	#
	#	calculate total number of windows from trees 
	#	this is to fill in missed pairs that are disconnected in all windows
	#
	cat('\ncalculating total number of windows for each pair (may take a while)')
	tmp			<- dtrms[, 
			{
				ptyr<- PTY_RUN
				z	<- subset(dtrees, PTY_RUN==ptyr)
				#	collect individuals in each tree of this phylotype run
				z	<- z[, {
							ph	<- phs[[IDX]]				
							tmp	<- regmatches(ph[['tip.label']], regexpr(regex.tip, ph[['tip.label']]))
							tmp	<- unique(regmatches(tmp, regexpr(regex.ind, tmp)))
							list(IND=tmp)
						}, by=c('IDX','W_FROM','W_TO')]
				#	expand to pairs
				dp			<- t( combn( z[, unique(IND)], 2 ) )
				colnames(dp)<- c('ID1','ID2')
				dp	<- as.data.table(dp)
				#	see in how many windows both individuals occur
				dp	<- dp[, {
							#IND1<- '15861_1_37'
							#IND2<- '15172_1_43'
							tmp	<- nrow(merge(subset(z, IND==ID1, c(W_FROM, W_TO)), subset(z, IND==ID2, c(W_FROM, W_TO)),by=c('W_FROM','W_TO')))
							list(WIN_TOTAL=tmp)
						}, by=c('ID1','ID2')]
				dp
			}, by='PTY_RUN']
	#	update dtrms with updated disconnected and updated total windows
	set(dtrms, NULL, c('SCORE','PAIR_ID','WIN_TOTAL'), NULL)
	dtrms	<- subset(dtrms, TYPE!='disconnected')
	tmp2	<- copy(dtrms)
	setnames(tmp2, c('ID1','ID2'), c('ID2','ID1'))
	set(tmp2, tmp2[, which(TYPE=='trans_12')], 'TYPE', 'trans')
	set(tmp2, tmp2[, which(TYPE=='trans_21')], 'TYPE', 'trans_12')
	set(tmp2, tmp2[, which(TYPE=='trans')], 'TYPE', 'trans_21')
	dtrms	<- rbind(dtrms, tmp2, use.names=TRUE)
	dtrms	<- dcast.data.table(dtrms, PTY_RUN+ID1+ID2~TYPE, value.var='WIN_OF_TYPE')
	dtrms	<- merge(tmp, dtrms, all.x=1, by=c('PTY_RUN','ID1','ID2'))
	#	re-calculate disconnected
	dtrms	<- melt(dtrms, id.vars=c('PTY_RUN','ID1','ID2','WIN_TOTAL'), variable.name='TYPE', value.name='WIN_OF_TYPE')	
	tmp		<- dtrms[, list(TYPE='disconnected', WIN_OF_TYPE=WIN_TOTAL[1]-sum(WIN_OF_TYPE, na.rm=TRUE)), by=c('PTY_RUN','ID1','ID2','WIN_TOTAL')]
	stopifnot(tmp[, all(WIN_OF_TYPE>=0)])
	dtrms	<- rbind(dtrms, tmp, use.names=TRUE)
	dtrms	<- subset(dtrms, !is.na(WIN_OF_TYPE))
	#	set pair id	
