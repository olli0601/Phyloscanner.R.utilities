...
```{r,eval=FALSE}
Rscript ~/Phyloscanner.R.utilities/misc_data_analysis_RCCS1519/software/make_trees.R --out_dir_base "/rds/general/project/ratmann_deepseq_analyses/live/mysecondrun/" --pkg_dir "~/Phyloscanner.R.utilities/misc_data_analysis_RCCS1519/software/" --prog_dir  "~/phyloscanner/phyloscanner_make_trees.py"  --sliding_width 25 --n_control 3 --cluster_size 55
```
...
### TSI estimates
```{r,eval=FALSE}
Rscript ~/Phyloscanner.R.utilities/misc_data_analysis_RCCS1519/software/make_deep_sequence_alignments.R --out_dir_base "/rds/general/project/ratmann_deepseq_analyses/live/myfirstrun/" --pkg_dir "~/Phyloscanner.R.utilities/misc_data_analysis_RCCS1519/software/"   --iqtree_option '-m GTR+F+R6'
```
`--out_dir_base` has a subdirectory **X_phsc_output** that stores the deep sequence alignment and will store the trees generated in this step. The scripts also allow the arguments  `--seed` (random seeds), `--env_name` (users' environment name) and `--date` (prefix of **X_phsc_output**). This command generates scripts to build trees for each deep sequence alignment, and the main outputs is in the form of **ptyrZ_InWindow_to_END_v2.treefile** **ptyrZ_InWindow_PositionsExcised_START_to_END_v2.treefile** depending on the form of the corresponding *fasta* files. 

### Transmission pair analysis
In the transmission pair analysis, we root the tree by *REF_CON_H*. 
```{r,eval=FALSE}
Rscript ~/Phyloscanner.R.utilities/misc_data_analysis_RCCS1519/software/make_deep_sequence_alignments.R --out_dir_base "/rds/general/project/ratmann_deepseq_analyses/live/mysecondrun/" --pkg_dir "~/Phyloscanner.R.utilities/misc_data_analysis_RCCS1519/software/"   --iqtree_option '-m GTR+F+R6 -o REF_CON_H'
```
