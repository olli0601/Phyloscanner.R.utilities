...
#
tmp <- Sys.info()
.f <- function(x, path) fifelse(is.na(x), path, x)

if(tmp["user"] == "xx4515")
{
        args$out.dir <- .f(args$out.dir, "/rds/general/project/ratmann_deepseq_analyses/live/test_full/")
...
today <- format(Sys.time(), '%d-%m-%y') == '27-09-22'
if( today )
{
...
dir.rccs1519 <- '/rds/general/project/ratmann_xiaoyue_jrssc2022_analyses/live/PANGEA2_RCCS1519_UVRI' 
if(today)
{
        dir.rccs1519 <- '/home/andrea/Documents/Box/ratmann_xiaoyue_jrssc2022_analyses/live/PANGEA2_RCCS1519_UVRI'
        dir.data <-  '/home/andrea/Documents/Box/ratmann_pangea_deepsequencedata'
}

infile.couple <-  file.path(dir.rccs1519, 'RakaiPangeaMetaData_v2.rda')
infile.ind.rccs <-  file.path(dir.data, 'PANGEA2_RCCS/200316_pangea_db_sharing_extract_rakai.csv')
infile.ind.mrc <-  file.path(dir.data,'PANGEA2_MRC/200319_pangea_db_sharing_extract_mrc.csv')

args$infile.rccs <-  file.path(dir.data, 'PANGEA2_RCCS/200422_PANGEA2_RCCS_mapped_samples.rds')
args$infile.mrc <- file.path(dir.data, 'PANGEA2_MRC/200422_PANGEA2_MRCUVRI_mapped_samples.rds')
args$infile.pty.runs <-  file.path(dir.rccs1519, '210120_RCCSUVRI_phscinput_samples.rds')

out.dir <- file.path(args$out.dir, 'potential_network')
infile.threshold <- file.path(dir.rccs1519, "200929_SIMILARTY_windowsize500_batchsize10/threshold_5quantile_indep_exp5.rda")

file.exists(infile.threshold)
file.exists(infile.couple)
file.exists(infile.ind.rccs)
file.exists(infile.ind.mrc)

if(0)
{
        tmp <- new.env()
        load(infile.threshold, tmp)
        tmp$threshold
}
...
break_large_clusters <- function(idlarge, dflarge, ddist) 
{
        ...
        for (i in idlarge) {
                ...
                maxsize <- dflarge[IDCLU == i, unique(CLU_SIZE)]
                ddist_idlarge_ind <-  ddist[pt_id1 %in% idlarge_ind & pt_id2 %in% idlarge_ind]
                chain_idlarge_ind <-  graph.data.frame(ddist_idlarge_ind,
                                                       directed = FALSE,
                                                       vertices = NULL)
...
if (any(grepl('similarity([0-9]+)_window_([0-9]+).rds', files))) {
        ...
        # Compute window similarities across batches
        for (window in unique(df$WINDOW)) 
        {
                ...
                # Load similarities
                .f <- function(b,w)
                        readRDS(file.path(out.dir,paste0('similarity', b, '_window_', w, '.rds')))
                dfbatch <- df[WINDOW == window, .f(BATCH, WINDOW), by=c('BATCH', 'WINDOW')]
        ...
        infile.similarity <-  file.path(out.dir, paste0('similarity', unique(df$WINDOW), '.rda'))
}else{
        infile.similarity <-  list.files(out.dir, pattern = '*.rda', full.names = T)
...
if( file.exists(infile.couple)  )
{
        ...
        couple <- subset(couple, select=c('male.RCCS_studyid', 'female.RCCS_studyid', 'COUPLE'))
        ...
        filename <- file.path(out.dir, paste0('similarity_couple_minlen_', args$length_cutoff, '.rds'))
        if(!file.exists(filename) | args$overwrite)
        {
                ...
                for (file in infile.similarity) {
                        ...
                        dfbatch <- merge(dfbatch, ids,
                                         by.x = c('H1'), by.y = c('pangea_id1'), all.x = T)
                        setnames(ids, colnames(ids), gsub('1', '2', colnames(ids)))

                        dfbatch <-  merge( dfbatch, ids, all.x=T,
                                          by.x = c('H2'), by.y = c('pangea_id2'))
                        setnames(ids, colnames(ids), gsub('2', '', colnames(ids)))
                        dfbatch <- dfbatch[!is.na(pt_id1) & !is.na(pt_id2) & pt_id1 != pt_id2,]
                        dfbatch <- merge(dfbatch, couple, all.x=T,
                                         by = c('pt_id1', 'pt_id2'))
                        dfbatch <- dfbatch[COUPLE == 1, c('pt_id1', 'pt_id2', 'PERC')]
                        window <- as.numeric(gsub('similarity([0-9]+).rda', '\\1', basename(file)))
                        dfbatch[, WINDOW := window]

                        ans <- fifelse(exists(ans), rbind(ans, dfbatch), dfbatch)
                }

                # Save
                if (args$if_save_data) {
                        cat('Write similarity among couples to ', filename, '...\n')
                        saveRDS(ans, file = filename)
                }
                gc()
        }else{
                ans <- data.table(readRDS(filename))
        }


        filename=file.path(out.dir,paste0( 'similarity_couple_fitted_thresholds_minlen_', args$length_cutoff, '.rda')

        if (nrow(ans) != 0 & !file.exists(filename) & args$overwrite)
        {
                ...
                for (window in 1:max(ans$WINDOW)) 
                {
                        ...
                        tmp$N <- length(tmp$y)
                        fit[[window]] <- stan(model_code = stan_code,
                                              data = tmp,
                                              iter = args$iter,
                                              warmup = args$warmup,
                                              chains = 1,
                ...
                if (args$if_save_data) {
                        cat('Write the fitted models to ', filename, '...\n')
                ...
                for (window in 1:max(ans$WINDOW)) 
                {
                        cat('Process window ', window, '\n')
                        tmp0 <- fit[[window]]
                        pars <- rstan::extract(tmp0, pars = names(tmp0)[!grepl('lp_', names(tmp0))])
                        pars = do.call(cbind, pars)
                        rh =   c(rh,   max( summary(tmp0)$summary[, "Rhat"]) )
                        ness = c(ness, min( summary(tmp0)$summary[, "n_eff"]) )
                }
                ...
                for (window in 1:max(ans$WINDOW)) 
                {
                        cat('Process window ', window, '\n')
                        tmp0 <- fit[[window]]
                        tmp = data.table(summary(tmp0)$summary[1:5, c("2.5%", "50%", "97.5%")])
                        tmp[, VARIABLE := rownames(summary(tmp0)$summary)[1:5]]
                        tmp[, WINDOW := window]
                        df = rbind(df, tmp)
                }
                ...
                if (args$if_save_data) 
                {
                        filename0 <- file.path(out.dir, paste0( 'similarity_couple_fitted_model_parameters_minlen_', args$length_cutoff, '.rds'))
                        cat('Write the fitted model parameters to ', filename, '...\n')
                        saveRDS(df, file = filename0)
                }
                ...
                {
                        cat('Process window ', window, '\n')
                        pars <-rstan::extract(fit[[window]], pars = c('mu', 'sigma', 'theta'))
                        tmp <- c()
                ...
                .f <- function(x) quantile(x, probs=c(.025, .5, .975))
                threshold = lapply(threshold, .f) 

                if (args$if_save_data) 
                {
                        cat('Write the thresholds to ', filename, '...\n')
                        save(threshold, file=filename)
                }

        } else if(file.exists(filename))
        {
                infile.threshold <- filename
...
dwindows <- data.table(WINDOW = windows, START = windows_start, END = windows_end)

if(0)
{
        # All entries are lowercase in consensus here...
        filename <- gsub('.fasta$|.fas$', '_depths.rds', args$infile)
        .f <- function(x) any(grepl('A|C|G|T', x))
        tmp <- sapply(alignment, .f)
        any(tmp)
}

if(!file.exists(filename) | args$overwrite)
{
        .load.consensus <- function(path, prefix, cols=NULL)
        {
                readRDS(args$infile.rccs) |> names()
                mapping <- readRDS(path)
                dcons <- subset(mapping, select = c('PANGEA_ID', 'CONSENSUS', 'F'))
                dcons <- unique(dcons[CONSENSUS != "",])
                dcons[, PANGEA_ID := paste0(prefix, PANGEA_ID)]
                if(!is.null(cols))
                        dcons <- dcons[, ..cols]
                dcons
        }

        dconsensus <- rbind(.load.consensus(args$infile.rccs, 'RCCS_',  cols=c('PANGEA_ID', 'F')),
                            .load.consensus(args$infile.mrc, 'MRCUVRI_', cols=c('PANGEA_ID', 'F')) ) 
        ...
        #
        if (args$if_save_data) {
                cat('Write the depths to ', filename, '...\n')
                saveRDS(ddepth, file = filename)
        }

}else{
        ddepth <- data.table(readRDS(filename))
}
...
filename <- file.path(out.dir, 'average_similarity.rds')
if(!file.exists(filename)| args$overwrite)
{
        ...
        for (window in 1:length(windows_start)) 
        {
                ...
                ddepth_select <- unique(subset(ddepth_windows[WINDOW == window,], select = c('pangea_id','HD')))

                setnames(ddepth_select, colnames(ddepth_select), paste0(colnames(ddepth_select), '1'))
                dfbatch <- merge(
                              dfbatch,
                              ddepth_select,
                              by.x = 'H1',
                ...
                setnames(ddepth_select, colnames(ddepth_select), gsub('1', '2', colnames(ddepth_select)))
                dfbatch <- merge(
                              dfbatch,
                              ddepth_select,
                              by.x = 'H2',
                ...
                setnames(ddepth_select, colnames(ddepth_select), gsub('2', '', colnames(ddepth_select)))
                dfbatch <- dfbatch[HD1 == 1 & HD2 == 1,]

                # average similarity per sequence pair
                tmp <- unique(subset(dfbatch, select = c('H1', 'H2', 'PERC')))
                tmp <- tmp[, list(PERC = mean(PERC, na.rm = T)), by = c('H1', 'H2')]
                ...
                if (window == 1) 
                {
                        ...
                        df <- merge(df, tmp, by = c('H1', 'H2'), all = T)
                }
        ...
        if (args$if_save_data) {
                cat( 'Write the average similarity scores to ', filename, '... \n')
...
filename <- file.path(out.dir, 'support_close.rds')
if(!file.exists(filename) | args$overwrite)
{
        ...
        for (window in 1:length(windows_start)) 
        {
                ...
                dfbatch <- merge( dfbatch, ddepth_select,
                              by.x = 'H1', by.y = 'pangea_id1',
                              all.x = T)

                setnames(ddepth_select, colnames(ddepth_select), gsub('1', '2', colnames(ddepth_select)))
                dfbatch <- merge(dfbatch, ddepth_select,
                              by.x = 'H2', by.y = 'pangea_id2',
                              all.x = T)
                ...
                tmp <- unique(subset(dfbatch, select = c('H1', 'H2', 'CLOSE')))
                tmp <- tmp[, list(CLOSE = mean(CLOSE, na.rm = T)), by = c('H1', 'H2')]

                setnames(tmp, 'CLOSE', paste0('CLOSE', window))
                if (window == 1) {
                        dsupport <- copy(tmp)
                } else{
                        dsupport <- merge(dsupport, tmp,
                                          by = c('H1', 'H2'),
                                          all = T)
                }
                rm(list=setdiff(ls(), c('couple','ids','threshold','ddepth_windows',
        ...
        if (args$if_save_data) {
                ...
                cat( 'Write the supports for closeness to ', filename, '... \n')
                saveRDS(dsupport, file = filename)
                rm(list=setdiff(ls(), c('couple','ids','dsupport','args','out.dir',lsf.str())))
                gc()
        }
}else{
        dsupport <- data.table(readRDS(filename))
}


cat('---------- Classify the closeness between sequences ----------\n')
cols <- grep("CLOSE", names(dsupport))
dsupport[ ,DATA := rowSums(!is.na(.SD)), .SDcols = cols]
dsupport[ ,SUPPORT := rowSums(.SD, na.rm = T), .SDcols = cols]
...
# map
setnames(ids, colnames(ids), paste0(colnames(ids), '1'))
df <- merge( dsupport, ids, by.x = c('H1'), by.y = c('pangea_id1'), all.x = T)

setnames(ids, colnames(ids), gsub('1', '2', colnames(ids)))
df <- merge( df, ids, by.x = c('H2'), by.y = c('pangea_id2'), all.x = T)
...
#
if (args$if_save_data) {
        filename <- file.path(out.dir, paste0( 'close_pairs_window_cutoff',
                                              gsub('\\.', '_', args$window_cutoff),
                                              '.rds'))
        cat('Write the average similarity scores to ', filename, '... \n')
        saveRDS(close_pairs, file = filename)
}
...
filename <- file.path(out.dir, 'whole_genome_similarity_close_pairs.rds')
if(!file.exists(filename))
{
        ...
        dist <- data.table(readRDS(fileanme))
}
...
rtc    <- data.table(ID = V(chains)$name,
...
if(nrow(dflarge) !=0)
{
        ...
        broken_dflarge <- break_large_clusters(idlarge, dflarge, ddist)
        while (T) 
        {
                id_clu <- max(df$IDCLU)
                tmp <- broken_dflarge[CLU_SIZE <= args$cluster_size]
                tmp2 <- unique(subset(tmp, select = c('CLU_SIZE', 'IDCLU')))
                setkey(tmp2, IDCLU)
                ...
                tmp[, IDCLU2:=NULL]
                df <- rbind(df, tmp)
                dflarge <- broken_dflarge[CLU_SIZE > args$cluster_size]
                idlarge <- unique(dflarge$IDCLU)
                print(idlarge)
                if (length(idlarge) == 0)
                        break
                broken_dflarge <- break_large_clusters(idlarge, dflarge, ddist)
                if (length(unique(broken_dflarge$IDCLU)) == 1)
                        warning(paste0('The best maximum cluster size that can be achieved is ',unique(broken_dflarge$CLU_SIZE),'\n'))
                break
        }
}
...
if (args$if_save_data) 
{
        filename <- file.path(out.dir, paste0( 'clusters_cutoff', gsub('\\.', '', args$window_cutoff), '.rda'))
        cat('Write potential transmission networks to ', filename, '...\n')
...
tmp <-  tmp %>% mutate(PTY_SIZE2 = sum_reset_at(args$n_merge)(PTY_SIZE))
...
filename <- file.path(out.dir, 'whole_genome_average_similarity.rds')
if(!file.exists(filename))
{
        df <- data.table(readRDS(file.path(out.dir, 'average_similarity.rds')))
        ...
        df <- merge(df,
                    ...
                    all.x = TRUE)
        setnames(ids, colnames(ids), gsub('2', '', colnames(ids)))
        df <-  df[, list(SIMILARITY = mean(SIMILARITY)), by = c('pt_id1', 'pt_id2')]
...
if (args$n_control != 0) 
{
        dcl <- rtc[, {
                ...
                list(
                     variable = c(paste0('ID_CLOSE', 1:args$n_control),
        ...
        if (args$if_save_data) 
        {
                filename <- file.path( out.dir, paste0(args$n_control, 'closest_individuals.rds'))
                cat('Write to ', filename , '...\n')
                saveRDS(dcl, file =  filename)
        }
}

tmp <- subset(dcl, select = c('CLU_SIZE', 'IDCLU', paste0('ID_CLOSE', 1:args$n_control)))
...
if (args$verbose) {
        cat(max(rtc$IDCLU),
            ' clusters of ', length(unique(rtc$ID)),
...
if (args$if_save_data) 
{
        outfile <- file.path(args$out.dir,
                             paste0('phscinput_runs_clusize_', args$cluster_size,'_ncontrol_', 
                                    args$n_control, '_windowcutoff_', args$window_cutoff,'.rds'))
