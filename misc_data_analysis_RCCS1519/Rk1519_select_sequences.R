...
rk.seq.get.best.mappeable.samples.Rakai2 <- function()
{
	require(data.table)
	
	infile.best.mappeable <- "~/Box/OR_Work/PANGEA/2020_PANGEA_UGanalyses/metadata_PANGEA/200422_PANGEA2_RCCS_selected_samples.rds"
	infile.extract <- "~/Box/OR_Work/PANGEA/2020_PANGEA_UGanalyses/metadata_PANGEA/200316_pangea_db_sharing_extract_rakai.csv"
	
	dbm <- readRDS(infile.best.mappeable)
	
	de <- as.data.table(read.csv(infile.extract, stringsAsFactors=FALSE))
	tmp <- subset(de, select=c(pt_id, pangea_id))
	tmp <- unique(tmp)
	setnames(tmp, colnames(tmp), toupper(colnames(tmp)))
	tmp <- merge(unique(subset(dbm, select=PANGEA_ID)), tmp, by='PANGEA_ID', all.x=TRUE)
	
	dbm <- merge(tmp, dbm, by='PANGEA_ID')
	write.csv(dbm, file= "~/Box/OR_Work/PANGEA/2020_PANGEA_UGanalyses/metadata_PANGEA/200422_PANGEA2_RCCS_selected_samples.csv", row.names=FALSE)
}
