...
if ( args$split_jobs_by_n == 1 | nrow(djob) <= 1000 )
{
  ...
  person_to_run <- rep(1:args$split_jobs_by_n, length.out = nrow(djob2))
  ...
  for (person in 1:args$split_djobs_list){
    ...
    pbs_file_person <- submit_jobs_from_djob(djob_person, output_type = "outfile", prefix = "srx")

    # TODO: add something that changes the PBS specifications for each person.

    write(pbs_file_person, 
            file = file.path(args$out.dir.work, paste0('submit_jobs_', person, '.sh')))
  }

    # TODO: write a script that each person can submit:
