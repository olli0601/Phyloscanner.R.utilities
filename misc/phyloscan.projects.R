...
project.Rakai.FastTree.170704<- function()
{
	...
	{			
		...
		#	run on HPC
		cmd				<- cmd.hpcwrapper.cx1.ic.ac.uk(hpc.walltime=998, hpc.q="pqeelab", hpc.mem="5900mb",  hpc.nproc=1, hpc.load='module load R/3.3.2')
		cmd				<- paste(cmd,tmp,sep='\n')
		cat(cmd)					
...
pty.pipeline.phyloscanner.170301.firstbatchofall.rerun<- function() 
{
	...
	if(1)
	{
		...
		#pty.c[1,cat(CMD)]
		invisible(pty.c[,	{					
							#cmd			<- cmd.hpcwrapper.cx1.ic.ac.uk(hpc.walltime=21, hpc.q="pqeelab", hpc.mem="5900mb",  hpc.nproc=1, hpc.load=hpc.load)							
							cmd		<- cmd.hpcwrapper.cx1.ic.ac.uk(hpc.walltime=21, hpc.q="pqeph", hpc.mem="3600mb",  hpc.nproc=1, hpc.load=hpc.load)
