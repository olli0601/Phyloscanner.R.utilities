...
RakaiFull.phylogeography.180618.map.Africa<- function()
{
	library(maps)
	library(mapdata)
	pdf('~/Dropbox (SPH Imperial College)/2017_phyloscanner_validation/figure_2_africa.pdf', w=8, h=8)
	map("worldHires", xlim=c(-25,50), ylim=c(-35,40), col='gray80', fill=TRUE)
	dev.off()	
}	
...
RakaiFull.phylogeography.190327.gender.mobility.data<- function(infile.inference=NULL)
{
	...
	tmp[, DUMMY:=NULL]
	#	define inmigrated in last year, and in last two years
	tmp[, MALE_INMIGRATE_05YR:= as.character(as.integer(MALE_TIME_INMIGRATED<=0.5))]
	tmp[, MALE_INMIGRATE_1YR:= as.character(as.integer(MALE_TIME_INMIGRATED<=1))]
	tmp[, MALE_INMIGRATE_2YR:= as.character(as.integer(MALE_TIME_INMIGRATED<=2))]
	tmp[, MALE_INMIGRATE_3YR:= as.character(as.integer(MALE_TIME_INMIGRATED<=3))]
	tmp[, MALE_INMIGRATE_4YR:= as.character(as.integer(MALE_TIME_INMIGRATED<=4))]
	tmp[, FEMALE_INMIGRATE_05YR:= as.character(as.integer(FEMALE_TIME_INMIGRATED<=0.5))]
	tmp[, FEMALE_INMIGRATE_1YR:= as.character(as.integer(FEMALE_TIME_INMIGRATED<=1))]
	tmp[, FEMALE_INMIGRATE_2YR:= as.character(as.integer(FEMALE_TIME_INMIGRATED<=2))]	
	tmp[, FEMALE_INMIGRATE_3YR:= as.character(as.integer(FEMALE_TIME_INMIGRATED<=3))]
	tmp[, FEMALE_INMIGRATE_4YR:= as.character(as.integer(FEMALE_TIME_INMIGRATED<=4))]
	...
	tmp		<- subset(tmp, select=c(PAIRID, DATE_FIRSTCONCPOS, 
										MALE_INMIG_LOC, MALE_INMIGRATE_05YR, MALE_INMIGRATE_1YR, MALE_INMIGRATE_2YR, MALE_INMIGRATE_3YR, MALE_INMIGRATE_4YR, MALE_INMIG_LON, MALE_INMIG_LAT, MALE_INMIGRATE_ADMIN,
										FEMALE_INMIG_LOC, FEMALE_INMIGRATE_05YR, FEMALE_INMIGRATE_1YR, FEMALE_INMIGRATE_2YR, FEMALE_INMIGRATE_3YR, FEMALE_INMIGRATE_4YR, FEMALE_INMIG_LON, FEMALE_INMIG_LAT, FEMALE_INMIGRATE_ADMIN))
	#	make compound variable
	...
	tmp		<- c(31.3,-0.34)
	tmp2	<- rtr3[, which(grepl('external',TR_INMIGRATE_05YR)|grepl('external',TR_INMIGRATE_1YR)|grepl('external',TR_INMIGRATE_2YR)|grepl('external',TR_INMIGRATE_3YR)|grepl('external',TR_INMIGRATE_4YR))]
	...
	set(rtr3, tmp2, 'TR_INMIG_LON', tmp[1])
	set(rtr3, tmp2, 'TR_INMIG_LAT', tmp[2])
	#	define fake community IDs 0.5YR
	tmp		<- unique(subset(rtr3, !grepl('resident',TR_INMIGRATE_05YR), c(TR_INMIG_LON, TR_INMIG_LAT, TR_INMIG_LOC)))
	tmp[, TR_COMM_NUM_A_MIG_05YR:= paste0(substr(TR_INMIG_LOC,1,1),'mig',seq_len(nrow(tmp)))]
	rtr3	<- merge(rtr3, tmp, by=c('TR_INMIG_LON','TR_INMIG_LAT','TR_INMIG_LOC'), all.x=TRUE)
	tmp		<- rtr3[, which(grepl('resident',TR_INMIGRATE_05YR))]
	set(rtr3, tmp, 'TR_COMM_NUM_A_MIG_05YR', rtr3[tmp,TR_COMM_NUM_A])
	#	define fake community IDs 1YR
	tmp		<- unique(subset(rtr3, !grepl('resident',TR_INMIGRATE_1YR), c(TR_INMIG_LON, TR_INMIG_LAT, TR_INMIG_LOC)))
	tmp[, TR_COMM_NUM_A_MIG_1YR:= paste0(substr(TR_INMIG_LOC,1,1),'mig',seq_len(nrow(tmp)))]
	rtr3	<- merge(rtr3, tmp, by=c('TR_INMIG_LON','TR_INMIG_LAT','TR_INMIG_LOC'), all.x=TRUE)
	tmp		<- rtr3[, which(grepl('resident',TR_INMIGRATE_1YR))]
	set(rtr3, tmp, 'TR_COMM_NUM_A_MIG_1YR', rtr3[tmp,TR_COMM_NUM_A])	
	#	define fake community IDs 2YR
	tmp		<- unique(subset(rtr3, !grepl('resident',TR_INMIGRATE_2YR), c(TR_INMIG_LON, TR_INMIG_LAT, TR_INMIG_LOC)))
	tmp[, TR_COMM_NUM_A_MIG_2YR:= paste0(substr(TR_INMIG_LOC,1,1),'mig',seq_len(nrow(tmp)))]
	rtr3	<- merge(rtr3, tmp, by=c('TR_INMIG_LON','TR_INMIG_LAT','TR_INMIG_LOC'), all.x=TRUE)
	tmp		<- rtr3[, which(grepl('resident',TR_INMIGRATE_2YR))]
	set(rtr3, tmp, 'TR_COMM_NUM_A_MIG_2YR', rtr3[tmp,TR_COMM_NUM_A])
	#	define fake community IDs 3YR
	tmp		<- unique(subset(rtr3, !grepl('resident',TR_INMIGRATE_3YR), c(TR_INMIG_LON, TR_INMIG_LAT, TR_INMIG_LOC)))
	tmp[, TR_COMM_NUM_A_MIG_3YR:= paste0(substr(TR_INMIG_LOC,1,1),'mig',seq_len(nrow(tmp)))]
	rtr3	<- merge(rtr3, tmp, by=c('TR_INMIG_LON','TR_INMIG_LAT','TR_INMIG_LOC'), all.x=TRUE)
	tmp		<- rtr3[, which(grepl('resident',TR_INMIGRATE_3YR))]
	set(rtr3, tmp, 'TR_COMM_NUM_A_MIG_3YR', rtr3[tmp,TR_COMM_NUM_A])
	#	define fake community IDs 4YR
	tmp		<- unique(subset(rtr3, !grepl('resident',TR_INMIGRATE_4YR), c(TR_INMIG_LON, TR_INMIG_LAT, TR_INMIG_LOC)))
	tmp[, TR_COMM_NUM_A_MIG_4YR:= paste0(substr(TR_INMIG_LOC,1,1),'mig',seq_len(nrow(tmp)))]
	rtr3	<- merge(rtr3, tmp, by=c('TR_INMIG_LON','TR_INMIG_LAT','TR_INMIG_LOC'), all.x=TRUE)
	tmp		<- rtr3[, which(grepl('resident',TR_INMIGRATE_4YR))]
	set(rtr3, tmp, 'TR_COMM_NUM_A_MIG_4YR', rtr3[tmp,TR_COMM_NUM_A])
	
	
	#	change locations of transmitter if transmitter is inmigrant and flow is not between same areas
	rtr3[, TR_LON_MIG_05YR:= TR_LONG]
	rtr3[, TR_LAT_MIG_05YR:= TR_LAT]	
	tmp		<- rtr3[, which(TR_INMIGRATE_05YR!='resident')]	
	set(rtr3, tmp, 'TR_LON_MIG_05YR', rtr3[tmp,TR_INMIG_LON])
	set(rtr3, tmp, 'TR_LAT_MIG_05YR', rtr3[tmp,TR_INMIG_LAT])
	rtr3[, TR_LON_MIG_1YR:= TR_LONG]
	rtr3[, TR_LAT_MIG_1YR:= TR_LAT]	
	tmp		<- rtr3[, which(TR_INMIGRATE_1YR!='resident')]	
	set(rtr3, tmp, 'TR_LON_MIG_1YR', rtr3[tmp,TR_INMIG_LON])
	set(rtr3, tmp, 'TR_LAT_MIG_1YR', rtr3[tmp,TR_INMIG_LAT])
	rtr3[, TR_LON_MIG_2YR:= TR_LONG]
	rtr3[, TR_LAT_MIG_2YR:= TR_LAT]	
	tmp		<- rtr3[, which(TR_INMIGRATE_2YR!='resident')]	
	set(rtr3, tmp, 'TR_LON_MIG_2YR', rtr3[tmp,TR_INMIG_LON])
	set(rtr3, tmp, 'TR_LAT_MIG_2YR', rtr3[tmp,TR_INMIG_LAT])
	rtr3[, TR_LON_MIG_3YR:= TR_LONG]
	rtr3[, TR_LAT_MIG_3YR:= TR_LAT]	
	tmp		<- rtr3[, which(TR_INMIGRATE_3YR!='resident')]	
	set(rtr3, tmp, 'TR_LON_MIG_3YR', rtr3[tmp,TR_INMIG_LON])
	set(rtr3, tmp, 'TR_LAT_MIG_3YR', rtr3[tmp,TR_INMIG_LAT])
	rtr3[, TR_LON_MIG_4YR:= TR_LONG]
	rtr3[, TR_LAT_MIG_4YR:= TR_LAT]	
	tmp		<- rtr3[, which(TR_INMIGRATE_4YR!='resident')]	
	set(rtr3, tmp, 'TR_LON_MIG_4YR', rtr3[tmp,TR_INMIG_LON])
	set(rtr3, tmp, 'TR_LAT_MIG_4YR', rtr3[tmp,TR_INMIG_LAT])
...
RakaiFull.phylogeography.181006.flows.wrapper<- function()
{
	...
	if(0)
	{
		infiles		<- c("~/Dropbox (SPH Imperial College)/Rakai Fish Analysis/full_run/RakaiAll_output_170704_w250_s20_p25_d50_stagetwo_rerun23_min10_withmetadata.rda"
		...
		for(ii in seq_along(infiles))
		{
			...
			RakaiFull.phylogeography.190327.gender.mobility.data(infile.inference)		
		}
	}
...
RakaiFull.phylogeography.181006.gender.mobility.thin.mcmc<- function(infile.inference.mcmc, mc.thin=NA)
{
	...
	if(is.na(mc.thin))
		mc.thin	<- 5
	tmp			<- seq.int(2, nrow(mc$pars$Z), mc$sweep*mc.thin)
	mc$pars$S	<- mc$pars$S[tmp,,drop=FALSE]
	mc$pars$Z	<- mc$pars$Z[tmp,,drop=FALSE]
	mc$pars$PI	<- mc$pars$PI[tmp,,drop=FALSE]
	mc$pars$N	<- mc$pars$N[tmp,,drop=FALSE]
...
RakaiFull.phylogeography.181006.mcmc.assess<- function(infile.inference=NULL, thin.extra=1)
{
	require(coda)
	if(is.null(infile.inference))
	{
		...
		infile.inference	<- "~/Dropbox (SPH Imperial College)/Rakai Fish Analysis/full_run/todi_pairs_181006_cl25_d50_prior23_min30_phylogeography_core_predict_mcmc_11101.rda"
	}			
	outfile.base		<- gsub('.rda$','',infile.inference)
	load(infile.inference)
	#	thin.extra=20
	if(!any(names(mc$pars)=='ETA'))
	{
		#	thin MCMC output
		tmp			<- seq.int(2, nrow(mc$pars$Z), mc$sweep*thin.extra)
		mc$pars$S	<- mc$pars$S[tmp,,drop=FALSE]
		mc$pars$Z	<- mc$pars$Z[tmp,,drop=FALSE]
		mc$pars$PI	<- mc$pars$PI[tmp,,drop=FALSE]
		mc$pars$N	<- mc$pars$N[tmp,,drop=FALSE]
	...
	if(any(names(mc$pars)=='ETA'))
	{
		...
		mc$pars$A		<- mc$pars$A[tmp,,drop=FALSE]
		mc$pars$ETA		<- mc$pars$ETA[tmp,,drop=FALSE]
		mc$pars$TOTALA	<- mc$pars$TOTALA[tmp,,drop=FALSE]
		gc()
		colnames(mc$pars$Z2)	<- paste0('Z2-',1:ncol(mc$pars$Z2))
		colnames(mc$pars$N2)	<- paste0('N2-',1:ncol(mc$pars$N2))
		colnames(mc$pars$ETA)	<- paste0('ETA-',1:ncol(mc$pars$ETA))
		colnames(mc$pars$A)		<- paste0('A-',1:ncol(mc$pars$A))
		colnames(mc$pars$TOTALA)<- 'TOTALA'
		#	convert to coda
		mcc			<- mcmc( cbind( mc$pars$Z2, mc$pars$N2, mc$pars$ETA, mc$pars$A, mc$pars$TOTALA) )		
	}
	mcc.eff		<- effectiveSize(mcc)
	#	plot trace and autocorrelations for variable with worst 6 effective sizes
	tmp			<- mcc.eff[mcc.eff>0] 
...
RakaiFull.phylogeography.190327.table1<- function()
{
	require(data.table)
	require(Hmisc)	
			
	infile.allpairs			<- "~/Dropbox (SPH Imperial College)/Rakai Fish Analysis/full_run/RakaiAll_output_190327_w250_s20_p25_d50_stagetwo_rerun23_min30_conf60_phylogeography_data_with_inmigrants.rda"
	outfile.base			<- gsub('_networksallpairs.rda','',infile.allpairs)
	...
	ansm	<- rbind(ansm,tmp,tmp2,fill=TRUE)
	ansm[, LABEL:= paste(COMM_TYPE, SEX, INMIG_2YRS_LOC, sep='-')]
	set(ansm, NULL, 'INMIG_2YRS_LOC', ansm[, factor(INMIG_2YRS_LOC, levels=c('Any','resident','inland','fish','external','origin_unknown'))])
	ansm	<- dcast.data.table(ansm, LABEL+COMM_TYPE+SEX+INMIG_2YRS_LOC~variable, value.var='LEGEND')
...
RakaiFull.phylogeography.181006.tablesupp.waifwm<- function()
{
	indir	<- '~/Dropbox (SPH Imperial College)/Rakai Fish Analysis/full_run'
	#	collect inference results for overall and by migration status
	infile	<- 'RakaiAll_output_181006_w250_s20_p25_d50_stagetwo_rerun23_min30_conf60_phylogeography_mcmc_11101_thinned1sweep_flows_fishinlandmigration.rda'
	outfile.base	<- file.path(indir, gsub('thinned1sweep_flows_fishinlandmigration.rda','',infile))
	load(file.path(indir,infile))	
	df		<- subset(ans, STAT=='waifm2', c(TR_COMM_TYPE, REC_COMM_TYPE, LABEL2))
	df[, ANALYSIS:= 'RCCS']
	setnames(df, c('TR_COMM_TYPE','REC_COMM_TYPE'), c('TR','REC'))
	
	infile	<- 'RakaiAll_output_181006_w250_s20_p25_d50_stagetwo_rerun23_min30_conf60_phylogeography_mcmc_11101_fishing_inland_predict.rda'
	load(file.path(indir,infile))	
	ans[, ANALYSIS:= 'prediction']
	setnames(ans, c('TR_AREA','REC_AREA'), c('TR','REC'))
	ans		<- subset(ans, STAT=='waifm', c(TR, REC, ANALYSIS, LABEL2))
	df		<- rbind(df, ans, fill=TRUE)
	
	set(df, NULL, 'TR', df[, factor(TR, levels=c('fisherfolk','inland','external'))])
	set(df, NULL, 'REC', df[, factor(REC, levels=c('fisherfolk','inland'))])	
	df		<- dcast.data.table(df, TR+REC~ANALYSIS, value.var='LABEL2')
	setkey(df, TR, REC)
	
	write.csv(df, row.names=FALSE, file=paste0(outfile.base, 'table_waifwm_overall.csv'))
}

RakaiFull.phylogeography.181006.tablesupp.sources<- function()
{
	indir	<- '~/Dropbox (SPH Imperial College)/Rakai Fish Analysis/full_run'
	#	collect inference results for overall and by migration status
	infile	<- 'RakaiAll_output_181006_w250_s20_p25_d50_stagetwo_rerun23_min30_conf60_phylogeography_mcmc_11101_thinned1sweep_flows_fishinlandmigration.rda'
	outfile.base	<- file.path(indir, gsub('thinned1sweep_flows_fishinlandmigration.rda','',infile))
	load(file.path(indir,infile))	
	df		<- subset(ans, STAT=='sources2', c(TR_COMM_TYPE, REC_COMM_TYPE, LABEL2))
	df[, ANALYSIS:= 'RCCS']
	setnames(df, c('TR_COMM_TYPE','REC_COMM_TYPE'), c('TR','REC'))
	...
	ans[, ANALYSIS:= 'prediction']
	setnames(ans, c('TR_AREA','REC_AREA'), c('TR','REC'))
	ans		<- subset(ans, STAT=='sources', c(TR, REC, ANALYSIS, LABEL2))
	df		<- rbind(df, ans, fill=TRUE)
	...
	write.csv(df, row.names=FALSE, file=paste0(outfile.base, 'table_sources_overall.csv'))
}
...
RakaiFull.phylogeography.181006.table2<- function()
{
	...
	#	collect observed counts by gender
	infile	<- file.path(indir,"RakaiAll_output_181006_w250_s20_p25_d50_stagetwo_rerun23_min30_conf60_phylogeography_data_with_inmigrants.rda")		
	load(infile)
	z	<- copy(rtr3)
	set(z, NULL, 'REC_COMM_TYPE', z[, as.character(factor(REC_COMM_TYPE=='fisherfolk', levels=c(TRUE,FALSE), labels=c('fisherfolk','inland')))])
	set(z, NULL, 'TR_COMM_TYPE', z[, as.character(factor(substr(TR_COMM_NUM_A_MIG,1,1), levels=c('f','a','t','i','e','u'), labels=c('fisherfolk','inland','inland2','inland3','external','unknown')))]) 
	set(z, z[, which(grepl('inland',TR_COMM_TYPE))],'TR_COMM_TYPE','inland')				
	#	count by gender
	z	<- z[, list(N=length(PAIRID), STAT='joint', ANALYSIS='count by gender'), by=c('TR_COMM_TYPE','REC_COMM_TYPE','TR_SEX','REC_SEX')]
	z[, M:= N/sum(N)]
	z[, LABEL2:= paste0(N,' (',round(100*M, d=1),'%)')]
	setnames(z, c('TR_COMM_TYPE','REC_COMM_TYPE'), c('TR','REC'))
	df		<- copy(z)
	#	count overall
	z	<- z[, list(N=sum(N), STAT='joint', ANALYSIS='count overall'), by=c('TR','REC')]	
	z[, M:= N/sum(N)]
	z[, LABEL2:= paste0(N,' (',round(100*M, d=1),'%)')]
	df		<- rbind(df, z, fill=TRUE)
	
	setnames(rtr2, c('TR_INMIGRATE_2YR','REC_INMIGRATE_2YR'), c('TR_INMIGRATE','REC_INMIGRATE'))	
	z	<- copy(rtr2)
	set(z, NULL, 'TR_COMM_TYPE', z[, as.character(factor(TR_COMM_TYPE=='fisherfolk', levels=c(TRUE,FALSE), labels=c('fisherfolk','inland')))])
	set(z, NULL, 'REC_COMM_TYPE', z[, as.character(factor(REC_COMM_TYPE=='fisherfolk', levels=c(TRUE,FALSE), labels=c('fisherfolk','inland')))])
	...
	set(z, z[, which(TR_INMIGRATE=='inmigrant_from_external')], 'TR_COMM_TYPE', 'external')
	set(z, z[, which(TR_INMIGRATE=='resident')], 'TR_INMIGRATE', 'resident/outmigrant')	
	set(z, z[, which(TR_INMIGRATE=='inmigrant_from_unknown')], 'TR_COMM_TYPE', 'unknown')
	...
	outfile.base	<- file.path(indir, gsub('thinned1sweep_flows_fishinlandmigration.rda','',infile))
	load(file.path(indir,infile))	
	ans[, ANALYSIS:= as.character(factor(grepl('2',STAT), levels=c(TRUE,FALSE), labels=c('inference overall','inference by migration status')))]
	set(ans, NULL, 'STAT', ans[,gsub('waifm2','waifm',gsub('joint2','joint',STAT))])
	setnames(ans, c('TR_COMM_TYPE','REC_COMM_TYPE'), c('TR','REC'))
	df		<- rbind(df, ans, fill=TRUE)
	
	#	collect inference results by gender
	infile	<- 'RakaiAll_output_181006_w250_s20_p25_d50_stagetwo_rerun23_min30_conf60_phylogeography_mcmc_11101_thinned1sweep_fishing_inland_gender_results.rda'
	load(file.path(indir,infile))
	ans[, ANALYSIS:= 'inference by gender']
	setnames(ans, c('TR_COMM_TYPE','REC_COMM_TYPE'), c('TR','REC'))
	df		<- rbind(df, ans, fill=TRUE)
	
	#	collect prediction results for overall
	infile	<- 'RakaiAll_output_181006_w250_s20_p25_d50_stagetwo_rerun23_min30_conf60_phylogeography_mcmc_11101_fishing_inland_predict.rda'
	load(file.path(indir,infile))
	ans[, ANALYSIS:= 'prediction overall']
	setnames(ans, c('TR_AREA','REC_AREA'), c('TR','REC'))
	df		<- rbind(df, ans, fill=TRUE)
	
	#	collect prediction results by gender
	infile	<- 'RakaiAll_output_181006_w250_s20_p25_d50_stagetwo_rerun23_min30_conf60_phylogeography_mcmc_11101_fishing_inland_gender_predict.rda'
	load(file.path(indir,infile))
	ans[, ANALYSIS:= 'prediction by gender']
	setnames(ans, c('TR_AREA','REC_AREA'), c('TR','REC'))
	df		<- rbind(df, ans, fill=TRUE)
	
	#	make pretty		
	set(df, NULL, 'TR', df[, factor(TR, levels=c('fisherfolk','inland','external','unknown'))])
	set(df, NULL, 'REC', df[, factor(REC, levels=c('fisherfolk','inland'))])
	
	#	select for main table
	df2	<- subset(df, grepl('overall',ANALYSIS) & STAT=='joint')
	df2	<- dcast.data.table(df2, TR+REC~ANALYSIS, value.var='LABEL2')
	write.csv(df2, row.names=FALSE, file=paste0(outfile.base, 'table_joint_by_overall.csv'))
	
	#	select for supp table by gender
	df2	<- subset(df, grepl('gender',ANALYSIS) & STAT=='joint')
	df2	<- dcast.data.table(df2, TR_SEX+TR+REC_SEX+REC~ANALYSIS, value.var='LABEL2')
	write.csv(df2, row.names=FALSE, file=paste0(outfile.base, 'table_joint_by_gender.csv'))
}


RakaiFull.phylogeography.181006.flows.fishinland.predict<- function(infile.predict=NULL)
{
	require(data.table)		
	if(is.null(infile.predict))
	{
		infile.predict	<- "~/Dropbox (SPH Imperial College)/Rakai Fish Analysis/full_run/RakaiAll_output_181006_w250_s20_p25_d50_stagetwo_rerun23_min30_conf60_phylogeography_core_predict_mcmc_11101.rda"
	}			
	outfile.base		<- gsub('.rda$','',gsub('_core_predict','',infile.predict))
	load(infile.predict)
	
	#	thin MCMC output
	burn.in	<- mc$it.info2[, max(IT)/2]
	tmp		<- mc$it.info2[, which(BLOCK=='ETA' & IT>burn.in)]
	tmp		<- tmp[ seq.int(1, length(tmp), by=3) ]
	mc$pars$A				<- mc$pars$A[tmp,,drop=FALSE]
	mc$pars$TOTALA			<- mc$pars$TOTALA[tmp,,drop=FALSE]
	mc$pars$ETA				<- mc$pars$ETA[tmp,,drop=FALSE]
	mc$pars$Z2				<- mc$pars$Z2[tmp,,drop=FALSE]
	mc$pars$N2				<- mc$pars$N2[tmp,,drop=FALSE]
	mc$it.info2				<- mc$it.info2[tmp,] 
	gc()
	colnames(mc$pars$ETA)	<- paste0('ETA-',1:ncol(mc$pars$ETA))
	
	#
	#	prepare data.table of proportions
	mcpi	<- as.data.table(mc$pars$ETA)
	mcpi[, IT:= seq_len(nrow(mcpi))]
	mcpi	<- melt(mcpi, id.vars='IT', variable.name='AREA_ID', value.name='ETA')
	set(mcpi, NULL, 'AREA_ID', mcpi[, gsub('([A-Z]+)-([0-9]+)','\\2',AREA_ID)])
	set(mcpi, NULL, 'AREA_ID', mcpi[, as.integer(AREA_ID)])
	tmp		<- subset(dc, select=c(REC_AREA, REC_SEX, TR_AREA, TR_SEX, AREA_ID))
	mcpi	<- merge(tmp, mcpi, by='AREA_ID')
	...
	#	geography who infects whom matrix  between fisherfolk and others
	#	adjusted P
	z		<- mcpi[, list(ETA=sum(ETA)), by=c('REC_AREA','TR_AREA','IT')]
	z		<- z[, list(P=qsn, Q=unname(quantile(ETA, p=qs))), by=c('REC_AREA','TR_AREA')]
	z		<- dcast.data.table(z, TR_AREA+REC_AREA~P, value.var='Q')
	z[, LABEL:= paste0(round(M*100, d=1), '%\n[',round(CL*100,d=1),'% - ',round(CU*100,d=1),'%]')]
	z[, LABEL2:= paste0(round(M*100, d=1), '% (',round(CL*100,d=1),'%-',round(CU*100,d=1),'%)')]
	setkey(z, REC_AREA, TR_AREA)
	z[, STAT:='joint']
	z[, DUMMY:= seq_len(nrow(z))]
	ans		<- copy(z)	
	...
	z		<- lapply(groups, function(group)
			{												
				z		<- subset(mcpi, TR_AREA==group)
				z		<- z[, list(ETA=sum(ETA)), by=c('REC_AREA','IT')]
				z		<- z[, list(REC_AREA=REC_AREA, ETA=ETA/sum(ETA)), by=c('IT')]				
				z		<- z[, list(P=qsn, Q=unname(quantile(ETA, p=qs))), by='REC_AREA']
				z[, TR_AREA:= group]
				z
			})
	z		<- do.call('rbind',z)
	...
	z		<- lapply(groups, function(group)
			{				
				z		<- subset(mcpi , REC_AREA==group)
				z		<- z[, list(ETA=sum(ETA)), by=c('TR_AREA','IT')]
				z		<- z[, list(TR_AREA=TR_AREA, ETA=ETA/sum(ETA)), by=c('IT')]
				z		<- z[, list(P=qsn, Q=unname(quantile(ETA, p=qs))), by='TR_AREA']
				z[, REC_AREA:= group]
			})
	z		<- do.call('rbind',z)
	z		<- dcast.data.table(z, TR_AREA+REC_AREA~P, value.var='Q')
	...
	save(ans, file=paste0(outfile.base,'_fishing_inland_predict.rda'))
}
...
{	
	...
	if(is.null(infile.inference))
	{
		infile.inference	<- "~/Dropbox (SPH Imperial College)/Rakai Fish Analysis/full_run/RakaiAll_output_181006_w250_s20_p25_d50_stagetwo_rerun23_min30_conf60_phylogeography_core_inference_mcmc_11101_thinned1sweep.rda"
	}			
	outfile.base		<- gsub('.rda$','',gsub('_core_inference','',infile.inference))
	load(infile.inference)
	
	#	thin MCMC output
	if(!grepl('thinned',infile.inference))
	{
		tmp			<- seq.int(2, nrow(mc$pars$Z), mc$sweep)
		mc$pars$S	<- mc$pars$S[tmp,,drop=FALSE]
		mc$pars$Z	<- mc$pars$Z[tmp,,drop=FALSE]
...
RakaiFull.phylogeography.181006.flows.fishinlandgender.predict<- function(infile.predict=NULL)
{	
	require(data.table)	
	require(Hmisc)	
	...
	#	adjusted P
	z		<- copy(mcpi)	
	z		<- z[, list(ETA=sum(ETA)), by=c('REC_AREA','TR_AREA','TR_SEX','REC_SEX','IT')]
	z		<- z[, list(P=qsn, Q=unname(quantile(ETA, p=qs))), by=c('TR_AREA','TR_SEX','REC_AREA','REC_SEX')]
	z		<- dcast.data.table(z, TR_AREA+REC_AREA+TR_SEX+REC_SEX~P, value.var='Q')
	z[, LABEL:= paste0(round(M*100, d=1), '%\n[',round(CL*100,d=1),'% - ',round(CU*100,d=1),'%]')]
	z[, LABEL2:= paste0(round(M*100, d=1), '% (',round(CL*100,d=1),'%-',round(CU*100,d=1),'%)')]
	setkey(z, TR_AREA, REC_AREA, TR_SEX, REC_SEX )
	z[, STAT:='joint']
	z[, DUMMY:= seq_len(nrow(z))]
	ans		<- copy(z)	
	z		<- NULL	
	...
	#	WAIFM
	#
	groups	<- data.table(TR_AREA=c('inland','inland','fisherfolk','fisherfolk'), TR_SEX=c('M','F','M','F'))
	z		<- lapply(1:nrow(groups), function(ii)
			{												
				z		<- copy(mcpi)
				z		<- subset(z, TR_AREA==groups$TR_AREA[ii] & TR_SEX==groups$TR_SEX[ii])
				z		<- z[, list(ETA=sum(ETA)), by=c('REC_AREA','REC_SEX','IT')]
				z		<- z[, list(REC_AREA=REC_AREA, REC_SEX=REC_SEX, ETA=ETA/sum(ETA)), by=c('IT')]				
				z		<- z[, list(P=qsn, Q=unname(quantile(ETA, p=qs))), by=c('REC_AREA','REC_SEX')]
				z[, TR_AREA:= groups$TR_AREA[ii] ]
				z[, TR_SEX:= groups$TR_SEX[ii] ]
				z
			})
	z		<- do.call('rbind',z)
	...
	z[, LABEL:= paste0(round(M*100, d=1), '%\n[',round(CL*100,d=1),'% - ',round(CU*100,d=1),'%]')]
	z[, LABEL2:= paste0(round(M*100, d=1), '% (',round(CL*100,d=1),'%-',round(CU*100,d=1),'%)')]
	setkey(z, TR_AREA, TR_SEX, REC_AREA, REC_SEX)
	z[, STAT:='waifm']
	z[, DUMMY:= seq_len(nrow(z))]
	ans		<- rbind(ans,z)	
	z		<- NULL	
	...
	#	sources
	#
	groups	<- data.table(REC_AREA=c('inland','inland','fisherfolk','fisherfolk'), REC_SEX=c('M','F','M','F'))
	z		<- lapply(1:nrow(groups), function(ii)
			{		
				z		<- copy(mcpi)
				z		<- subset(z, REC_AREA==groups$REC_AREA[ii] & REC_SEX==groups$REC_SEX[ii])
				z		<- z[, list(ETA=sum(ETA)), by=c('TR_AREA','TR_SEX','IT')]
				z		<- z[, list(TR_AREA=TR_AREA, TR_SEX=TR_SEX, ETA=ETA/sum(ETA)), by=c('IT')]
				z		<- z[, list(P=qsn, Q=unname(quantile(ETA, p=qs))), by=c('TR_AREA','TR_SEX')]
				z[, REC_AREA:= groups$REC_AREA[ii] ]
				z[, REC_SEX:= groups$REC_SEX[ii] ]
			})
	z		<- do.call('rbind',z)
	z		<- dcast.data.table(z, TR_AREA+REC_AREA+TR_SEX+REC_SEX~P, value.var='Q')
	z[, LABEL:= paste0(round(M*100, d=1), '%\n[',round(CL*100,d=1),'% - ',round(CU*100,d=1),'%]')]
	z[, LABEL2:= paste0(round(M*100, d=1), '% (',round(CL*100,d=1),'%-',round(CU*100,d=1),'%)')]
	setkey(z, REC_AREA, REC_SEX, TR_AREA, TR_SEX)
	z[, STAT:='sources']
	z[, DUMMY:= seq_len(nrow(z))]
	ans		<- rbind(ans,z)	
	z		<- NULL	
	...
	save(ans, file=paste0(outfile.base,'_fishing_inland_gender_predict.rda'))
}
...
RakaiFull.phylogeography.181006.flows.netflows<- function(infile.inference=NULL)
{
	...
	outfile.base		<- gsub('_thinned1sweep','',gsub('.rda$','',gsub('_core_inference','',infile.inference)))
	load(infile.inference)
	...
	setkey(z, SINK_TYPE, SINK_SEX )	
	ans		<- rbind(z, ans, fill=TRUE)
	setkey(ans, SINK_TYPE, SINK_MIGRATIONSTATUS, SINK_SEX)
	#	save
	save(ans, file=paste0(outfile.base,'_flows_netflows.rda'))
	write.csv(ans, file=paste0(outfile.base,'_flows_netflows.csv'), row.names=FALSE)
	#
	if(0)
	{
		ggplot(ans, aes(x=SINK_SEX, middle=M, min=CL, lower=IL, upper=IU, max=CU)) +
...
RakaiFull.phylogeography.181006.flows.netflows.predict<- function(infile.inference=NULL)
{
	require(data.table)	
	require(Hmisc)	
	...
	outfile.base		<- gsub('_thinned1sweep','',gsub('.rda$','',gsub('_core','',infile.inference)))
	load(infile.inference)
	...
	colnames(mc$pars$U)	<- paste0('U-',1:ncol(mc$pars$U))
	colnames(mc$pars$A)	<- paste0('A-',1:ncol(mc$pars$A))
	colnames(mc$pars$ETA)<- paste0('ETA-',1:ncol(mc$pars$ETA))
	colnames(mc$pars$N)	<- 'TOTALA'
	...
	z		<- copy(mcpi)
	z		<- z[, list(ETA=sum(ETA)), by=c('REC_AREA','TR_AREA','TR_SEX','REC_SEX','IT')]
	z[, FLOW:=paste0(TR_AREA,' ',TR_SEX,' ',REC_AREA,' ',REC_SEX)]
	z		<- dcast.data.table(z, IT~FLOW, value.var='ETA')	
	z[ , inlanddivfisherfolk_M:= z[['inland M fisherfolk F']] / z[['fisherfolk M inland F']] ]
	z[ , inlanddivfisherfolk_F:= z[['inland F fisherfolk M']] / z[['fisherfolk F inland M']] ]				
	z		<- melt(z, id.vars='IT', measure.vars=c('inlanddivfisherfolk_M','inlanddivfisherfolk_F'))
	z		<- z[, list(P=qsn, Q=unname(quantile(value, p=qs))), by=c('variable')]
	z[, SINK_TYPE:= gsub('([^_]+)_([^_]+)','\\1',variable)]
	z[, SINK_SEX:= gsub('([^_]+)_([^_]+)','\\2',variable)]	
	...
	#	add overall
	z		<- copy(mcpi)
	z		<- z[, list(ETA=sum(ETA)), by=c('REC_AREA','TR_AREA','IT')]
	z[, FLOW:=paste0(TR_AREA,' ',REC_AREA)]
	z		<- dcast.data.table(z, IT~FLOW, value.var='ETA')	
...
RakaiFull.phylogeography.181006.predict.areaflows.old1<- function()
{	
	require(data.table)	
	require(Boom)	
...
{	
	...
	mc$pars$NU			<- NA_real_															#prior for N
	mc$pars$Z2			<- matrix(NA_real_, ncol=length(unique(dc$AREA_ID)), nrow=mc$n)		#resample of augmented data (RCCS level) 
	mc$pars$N2			<- matrix(NA_real_, ncol= 1, nrow=mc$n)		#resample of total number of counts on augmented data (RCCS level)
	mc$it.info2			<- data.table(	IT= rep(NA_integer_, mc$n),
			SAMPLE_POSTERIOR= rep(NA_integer_, mc$n),
			PAR_ID= rep(NA_integer_, mc$n),
			BLOCK= rep(NA_character_, mc$n),
			MHRATIO= rep(NA_real_, mc$n),
	...
	for(ii in 1:10000)
	{
		...
		#
		#	copy from current mc to mc.skeleton
		#
		mc.skeleton$n				<- mc.skeleton$n + mc$n
		mc.skeleton$pars$ETA		<- rbind(mc.skeleton$pars$ETA, mc$pars$ETA)
		mc.skeleton$pars$A			<- rbind(mc.skeleton$pars$A, mc$pars$A)
		mc.skeleton$pars$TOTALA		<- rbind(mc.skeleton$pars$TOTALA, mc$pars$TOTALA)
		mc.skeleton$pars$Z2			<- rbind(mc.skeleton$pars$Z2, mc$pars$Z2)
		mc.skeleton$pars$N2			<- rbind(mc.skeleton$pars$N2, mc$pars$N2)
		mc.skeleton$pars$ETA_PRIOR	<- mc$pars$ETA_PRIOR
		mc.skeleton$pars$U			<- mc$pars$U
		mc.skeleton$pars$NU			<- mc$pars$NU
		mc.skeleton$it.info2		<- rbind(mc.skeleton$it.info2, mc$it.info2)		
	}
...
RakaiFull.phylogeography.190327.predict.areaflows.debug<- function()
{
	require(data.table)	
	require(gtools)	
	...
	if(is.null(infile.inference.data))	
		infile.inference.data			<- "~/Dropbox (SPH Imperial College)/Rakai Fish Analysis/full_run/todi_pairs_181006_cl25_d50_prior23_min30_phylogeography_data_with_inmigrants.rda"
	if(is.null(infile.inference.mcmc))
		infile.inference.mcmc			<- "~/Dropbox (SPH Imperial College)/Rakai Fish Analysis/full_run/RakaiAll_output_181006_w250_s20_p25_d50_stagetwo_rerun23_min30_conf60_phylogeography_samcmc190327_nsweep1e5_opt112401.rda"
	...
	#		
	#	calculate eligible in RCCS communities
	dsubdis	<- desm[, list(COMM_ELIGIBLE=sum(PART_EVER+PART_NEVER)), by=c('COMM_NUM','COMM_NUM_A','COMM_TYPE','LONG','LAT','SEX')]
	dsubdis	<- merge(dsubdis, data.table(COMM_NUM=c('16m','16m','51m','51m','24m','24m','22m','22m'), COMM_NUM2=c('107','16','776','51','4','24','1','22')), all=TRUE, by='COMM_NUM')	
	tmp		<- dsubdis[, which(is.na(COMM_NUM2))]
	set(dsubdis, tmp, 'COMM_NUM2', dsubdis[tmp, COMM_NUM])
	set(dsubdis, NULL, 'COMM_NUM2', dsubdis[, as.integer(COMM_NUM2)])	
	#	associate subdistrict names to communities	
	load(infile.subdistricts)
	tmp		<- subset(as.data.table(comgps2), select=c(COMM_NUM, loc))
	setnames(tmp, c('COMM_NUM','loc'), c('COMM_NUM2','SUBDISTRICT'))
	set(tmp, NULL, 'COMM_NUM2', tmp[, as.integer(as.character(COMM_NUM2))])
	dsubdis	<- merge(dsubdis, tmp, by='COMM_NUM2', all=TRUE)
	stopifnot( !any(is.na(dsubdis[,SUBDISTRICT])))
	#	define subdistrict type as discussed with Kate
	tmp		<- dsubdis[, list(	AREA=as.character(factor(any(COMM_TYPE=='fisherfolk'), levels=c(TRUE,FALSE), labels=c('fishing','inland')))), by=c('SUBDISTRICT')]	
	dsubdis	<- merge(tmp, dsubdis, by='SUBDISTRICT' )
	#	add further info on subdistricts
	tmp		<- as.data.table(subdis.data)
	setnames(tmp, colnames(tmp), toupper(gsub('_count','_popcount',gsub('\\.','_',gsub('subdis.name','SUBDISTRICT',colnames(tmp))))))
	set(tmp, NULL, c('SURVEY_HIVPREV','SURVEY_MALE_HIVPREV','SURVEY_FEMALE_HIVPREV','MAP_POPCOUNT','MAP_HIVCOUNT','MAP_HIVPREV'), NULL)
	...
	tmp		<- melt(tmp, id.vars='SUBDISTRICT', value.name='MAP_POPCOUNT', variable.name='SEX')
	set(tmp, NULL, 'SEX', tmp[, substr(gsub('MAP_([A-Z]+)_POPCOUNT','\\1',SEX),1,1)])
	dsubdis	<- merge(dsubdis, tmp, by=c('SUBDISTRICT','SEX'), all.x=TRUE)
	#	define population surveyed by area
	dprior	<- dsubdis[, list(	SUBDISTRICT_POP=MAP_POPCOUNT[1], POP_RCCS_YES=sum(COMM_ELIGIBLE), POP_RCCS_NO=MAP_POPCOUNT[1]-sum(COMM_ELIGIBLE)	), by=c('SUBDISTRICT','AREA','SEX')]
	dprior	<- dprior[, lapply(.SD, sum, na.rm=TRUE), by=c('AREA','SEX'), .SDcols=c("POP_RCCS_YES","POP_RCCS_NO") ] 
	dprior[, SAMPLING_CATEGORY:=paste0(AREA,':',SEX)]
	dprior[, P:= POP_RCCS_YES/(POP_RCCS_YES+POP_RCCS_NO)]
	dprior[, LP:= 0]
	dprior[, SAMPLE:= 1L]
	set(dprior, NULL, c('AREA','SEX','POP_RCCS_YES','POP_RCCS_NO'), NULL)
	...
	control	<- list(	burnin.p=0.05, 
			thin=NA_integer_, 
			regex_pars='*')
	mca		<- source.attribution.mcmc.aggregateToTarget(mcmc.file=infile.inference.mcmc, daggregateTo=daggregateTo, control=control)
	mca		<- subset(mca, !grepl('external',TR_TARGETCAT))
	setnames(mca, c('TR_TARGETCAT','REC_TARGETCAT'), c('TR_TRM_CAT','REC_TRM_CAT'))
	
	
	#
	#	aggregate MCMC samples to area of subdistricts
	#	
	daggregateTo				<- subset(dobsRCCS, select=c(TRM_CAT_PAIR_ID, TR_TRM_CATEGORY, REC_TRM_CATEGORY))
	daggregateTo[, TR_DUMMY:= gsub('^([a-z]+):([FM]):.*','\\1:\\2',daggregateTo$TR_TRM_CATEGORY)]
	daggregateTo[, REC_DUMMY:= gsub('^([a-z]+):([FM]):.*','\\1:\\2',daggregateTo$REC_TRM_CATEGORY)]
	tmp							<- unique(subset(dsubdis, select=c(AREA, SEX, COMM_NUM_A)))
	tmp							<- rbind(tmp, data.table(AREA='external', SEX=c('F','M'), COMM_NUM_A='emig'), data.table(AREA='inland', SEX=c('F','M'), COMM_NUM_A='imig'), data.table(AREA='fishing', SEX=c('F','M'), COMM_NUM_A='fmig'))
	tmp[, DUMMY:= paste0(COMM_NUM_A,':',SEX)] 
	tmp[, TARGETCAT:= paste0(AREA,':',SEX)]
	tmp							<- subset(tmp, select=c(DUMMY,TARGETCAT))
	setnames(tmp, colnames(tmp), paste0('TR_',colnames(tmp)))	
	if( length( daggregateTo$TR_DUMMY[ !daggregateTo$TR_DUMMY%in%tmp$TR_DUMMY ] )>0 )
		stop('Fatal error in aggregating MCMC transmitter categories to subdistrict areas')
	daggregateTo				<- merge(daggregateTo,tmp,by='TR_DUMMY')
	setnames(tmp, colnames(tmp), gsub('TR_','REC_',colnames(tmp)))	
	if( length( daggregateTo$REC_DUMMY[ !daggregateTo$REC_DUMMY%in%tmp$REC_DUMMY ] )>0 )
		stop('Fatal error in aggregating MCMC recipient categories to subdistrict areas')	
	daggregateTo				<- merge(daggregateTo, tmp, by='REC_DUMMY')
	set(daggregateTo, NULL, c('REC_DUMMY','TR_DUMMY','TR_TRM_CATEGORY','REC_TRM_CATEGORY'), NULL)	
	control	<- list(	burnin.p=0.05, 
			thin=NA_integer_, 
			regex_pars='*')
	mca		<- source.attribution.mcmc.aggregateToTarget(mcmc.file=infile.inference.mcmc, daggregateTo=daggregateTo, control=control)
	...
	dobs[, TRM_OBS:=NULL]
	tmp	<- subset(mca, VARIABLE=='Z')[, list(VALUE=round(mean(VALUE))), by=c('TR_TRM_CAT','REC_TRM_CAT')]
	set(tmp, NULL, 'TR_TRM_CAT', tmp[, factor(TR_TRM_CAT, levels=c('fishing:M','inland:M','fishing:F','inland:F'))])
	set(tmp, NULL, 'REC_TRM_CAT', tmp[, factor(REC_TRM_CAT, levels=c('fishing:M','inland:M','fishing:F','inland:F'))])
	setkey(tmp, TR_TRM_CAT, REC_TRM_CAT)
	tmp[, PI:= round(VALUE/sum(VALUE)*100,d=1)]
	
	
	tmp2	<- subset(mca, VARIABLE=='PI')[, list(VALUE=mean(VALUE)), by=c('TR_TRM_CAT','REC_TRM_CAT')]
	set(tmp2, NULL, 'VALUE', tmp2[, VALUE/sum(VALUE)])
	set(tmp2, NULL, 'TR_TRM_CAT', tmp2[, factor(TR_TRM_CAT, levels=c('fishing:M','inland:M','fishing:F','inland:F'))])
	set(tmp2, NULL, 'REC_TRM_CAT', tmp2[, factor(REC_TRM_CAT, levels=c('fishing:M','inland:M','fishing:F','inland:F'))])
	setkey(tmp2, TR_TRM_CAT, REC_TRM_CAT)
	
	tmp3	<- subset(mca, VARIABLE=='PI')[, list(VALUE=median(VALUE)), by=c('TR_TRM_CAT','REC_TRM_CAT')]
	set(tmp3, NULL, 'VALUE', tmp3[, VALUE/sum(VALUE)])
	set(tmp3, NULL, 'TR_TRM_CAT', tmp3[, factor(TR_TRM_CAT, levels=c('fishing:M','inland:M','fishing:F','inland:F'))])
	set(tmp3, NULL, 'REC_TRM_CAT', tmp3[, factor(REC_TRM_CAT, levels=c('fishing:M','inland:M','fishing:F','inland:F'))])
	setkey(tmp3, TR_TRM_CAT, REC_TRM_CAT)
	...
	if(0)
	{
		s		<- c(0.2363956, 0.2631666, 0.3919308, 0.5427133)
		#pi		<- c(0.243,0.029,0.039,0.231,0.184,0.006,0.039,0.127)
		pi		<- c(0.31125152, 0.02319007, 0.04359218, 0.22639770, 0.21429612, 0.00310487, 0.04633952, 0.13182802)
		pi		<- pi/sum(pi)
...
RakaiFull.phylogeography.190327.sensitivity.analyses.prep<- function()
{
	...
	for(i in seq_along(mcmc.files))
	{
		...
		aggregate.files	<- c( 	gsub('\\.rda','_aggregatedFishInland.csv',mcmc.file),
								gsub('\\.rda','_aggregatedFishInlandByGender.csv',mcmc.file))
		
		# if not all aggregate files exist, need to aggregate.
		if(!all(file.exists(aggregate.files)))
		{
			infile.inference	<- paste0(gsub('(^.*phylogeography_).*$','\\1',mcmc.file),'data_with_inmigrants.rda')
		...
		aggregate.file	<- gsub('\\.rda','_aggregatedFishInlandByGender.csv',mcmc.file)
		if(!file.exists(gsub('\\.csv','_flowsetc.csv',aggregate.file)))
		{
...
RakaiFull.phylogeography.190327.sensitivity.analyses.prediction<- function()
{
	require(data.table)
	require(scales)
	require(ggplot2)
	...
	infiles	<- subset(infiles, grepl('nsweep1e5',FI) & grepl('samcmc190327',FI) & !grepl('^todi',FI) & grepl('prAreas',FI) & !grepl('copy',FI))
	...
	infiles[, OPT_HIVCNT:= factor(substring(OPT,7,7), levels=c('1','0'), labels=c('Prop infected','Prop all'))]
	infiles[, OPT_INFL:= as.numeric(substring(OPT,8,9))/10]	
	infiles[, STRAT:= factor(grepl('Gender',FI), levels=c(FALSE,TRUE), labels=c('overall','by gender'))]
	
	se	<- vector('list', nrow(infiles))
	for(i in seq_len(nrow(infiles)))
	{
		tmp		<- as.data.table(read.csv(file.path(indir, infiles$FI[i]), stringsAsFactors=FALSE))
		for(x in c('STRAT','OPT_PART','OPT_SEQ','OPT_MIG','OPT_UNKMIG','OPT_HIVCNT','OPT_INFL','FI'))
	...
	#	
	df	<- copy(se)
	df[, SENS:= paste0(OPT_HIVCNT,', ', OPT_INFL,'X')]
	tmp	<- df[, which(SENS=='Prop all, 2X')]
	...
	ggplot( subset(df, STAT=='flows' & STRAT=='overall')) +
			...
			scale_colour_brewer(palette='Dark2') +
			theme_bw() +
			labs(y='\npredicted transmission flows\nwithin and between Rakai inland and fishing area',x='',colour='') +
			coord_flip()
	ggsave(file=paste0(outfile.base,tmp,'_flows_overall.pdf'), w=8, h=6)
	ggplot( subset(df, STAT=='flows' & STRAT=='by gender')) +
			...
			labs(y='\npredicted transmission flows\nwithin and between Rakai inland and fishing area',x='',colour='') +
			coord_flip()
	ggsave(file=paste0(outfile.base,tmp,'_flows_byGender.pdf'), w=8, h=8)
	...
	ggsave(file=paste0(outfile.base,tmp,'_flowratio_overall.pdf'), w=8, h=2.25)
...
RakaiFull.phylogeography.190327.sensitivity.analyses<- function()
{
	...
	#
	#	sensitivity: quality criteria on deep seq depth
	#
	df	<- subset(se, CONFCUT=='60%' & OPT_PART=='Yes' & OPT_SEQ=='Yes' & OPT_MIG=='24 months' & OPT_UNKMIG=='fishing communities')
	df[, SENS:= MINDEPTH]
	tmp	<- df[, which(SENS=='30X')]
	set(df, tmp, 'SENS', df[tmp, paste0(SENS,' (central)')])
	tmp	<- 'mindepth'
	#	
	ggplot( subset(df, STAT=='flows' & STRAT=='overall')) +
	...
	#
	#	sensitivity: confidence cut off on source-recipient pairs
	#
	df	<- subset(se, MINDEPTH=='30X' & OPT_PART=='Yes' & OPT_SEQ=='Yes' & OPT_MIG=='24 months' & OPT_UNKMIG=='fishing communities')
	df[, SENS:= CONFCUT]
	tmp	<- df[, which(SENS=='60%')]
	set(df, tmp, 'SENS', df[tmp, paste0(SENS,' (central)')])
	tmp	<- 'confcut'
	#	
	ggplot( subset(df, STAT=='flows' & STRAT=='overall')) +
	...
	#
	df	<- subset(se, MINDEPTH=='30X' & CONFCUT=='60%' & OPT_PART=='Yes' & OPT_SEQ=='Yes' & OPT_UNKMIG=='fishing communities')
	df[, SENS:= OPT_MIG]
	tmp	<- df[, which(SENS=='24 months')]
	set(df, tmp, 'SENS', df[tmp, paste0(SENS,' (central)')])
	...
	#
	#	sensitivity: migration missing origins
	#
	df	<- subset(se, MINDEPTH=='30X' & CONFCUT=='60%' & OPT_PART=='Yes' & OPT_SEQ=='Yes' & OPT_MIG=='24 months')
	df[, SENS:= OPT_UNKMIG]
	tmp	<- df[, which(SENS=='fishing communities')]
	set(df, tmp, 'SENS', df[tmp, paste0(SENS,' (central)')])
...
RakaiFull.phylogeography.190327.highway.corridor<- function()
{
	...
	if(opt$migration.def.code=='06')
	{
		...
		setnames(rtr, 'REC_INMIGRATE_05YR', 'REC_INMIGRATE')
		setnames(rtr, 'TR_COMM_NUM_A_MIG_05YR', 'TR_COMM_NUM_A_MIG')
		setnames(rtr, 'TR_LON_MIG_05YR', 'TR_LON')	
		setnames(rtr, 'TR_LAT_MIG_05YR', 'TR_LAT')	
	}
	if(opt$migration.def.code=='12')
	{
		cat('\nSource attribution analysis based on TR_INMIGRATE_1YR, REC_INMIGRATE_1YR, TR_COMM_NUM_A_MIG_1YR')
		...
		setnames(rtr, 'REC_INMIGRATE_1YR', 'REC_INMIGRATE')
		setnames(rtr, 'TR_COMM_NUM_A_MIG_1YR', 'TR_COMM_NUM_A_MIG')
		setnames(rtr, 'TR_LON_MIG_1YR', 'TR_LON')
		setnames(rtr, 'TR_LAT_MIG_1YR', 'TR_LAT')
	}
	if(opt$migration.def.code=='24')
	{
		cat('\nSource attribution analysis based on TR_INMIGRATE_2YR, REC_INMIGRATE_2YR, TR_COMM_NUM_A_MIG_2YR')
		...
		setnames(rtr, 'REC_INMIGRATE_2YR', 'REC_INMIGRATE')
		setnames(rtr, 'TR_COMM_NUM_A_MIG_2YR', 'TR_COMM_NUM_A_MIG')
		setnames(rtr, 'TR_LON_MIG_2YR', 'TR_LON')
		setnames(rtr, 'TR_LAT_MIG_2YR', 'TR_LAT')
	}
	if(opt$migration.def.code=='36')
	{
		cat('\nSource attribution analysis based on TR_INMIGRATE_3YR, REC_INMIGRATE_3YR, TR_COMM_NUM_A_MIG_3YR')
		...
		setnames(rtr, 'REC_INMIGRATE_3YR', 'REC_INMIGRATE')
		setnames(rtr, 'TR_COMM_NUM_A_MIG_3YR', 'TR_COMM_NUM_A_MIG')
		setnames(rtr, 'TR_LON_MIG_3YR', 'TR_LON')
		setnames(rtr, 'TR_LAT_MIG_3YR', 'TR_LAT')
	}
	if(opt$migration.def.code=='48')
	{
		cat('\nSource attribution analysis based on TR_INMIGRATE_4YR, REC_INMIGRATE_4YR, TR_COMM_NUM_A_MIG_4YR')
		...
		setnames(rtr, 'REC_INMIGRATE_4YR', 'REC_INMIGRATE')
		setnames(rtr, 'TR_COMM_NUM_A_MIG_4YR', 'TR_COMM_NUM_A_MIG')
		setnames(rtr, 'TR_LON_MIG_4YR', 'TR_LON')
		setnames(rtr, 'TR_LAT_MIG_4YR', 'TR_LAT')
	}
	
	rtr	<- subset(rtr, select=c(	'PAIRID','TR_RID','TR_COMM_NUM','TR_COMM_NUM_A','TR_COMM_NUM_A_MIG',
					'TR_SEX','TR_BIRTHDATE','TR_COMM_TYPE','TR_INMIG_LOC','TR_INMIGRATE', 'TR_LON', 'TR_LAT',
					'REC_RID','REC_COMM_NUM','REC_COMM_NUM_A',
					'REC_SEX','REC_BIRTHDATE','REC_COMM_TYPE','REC_INMIGRATE','REC_LONG','REC_LAT'))
	...
	set(rtr, NULL, 'TR_COMM_NUM_A_MIG', rtr[, gsub('[0-9]+','',TR_COMM_NUM_A_MIG)])
	#	set unknown origin to either fishing or inland
	rtr	<- subset(rtr, TR_INMIGRATE!='inmigrant_from_unknown')
	...
	#	calculate observed number of transmissions
	#
	dobs	<- rtr[, list( TRM_OBS=length(unique(PAIRID))), by=c('TR_TRM_CATEGORY','REC_TRM_CATEGORY','TR_SAMPLING_CATEGORY','REC_SAMPLING_CATEGORY','REC_COMM_NUM_A','TR_COMM_NUM_A_MIG')]
	setkey(dobs, TR_TRM_CATEGORY, REC_TRM_CATEGORY )	
	dobs[, TRM_CAT_PAIR_ID:= seq_len(nrow(dobs))]
	...
	#	get highway coords
	ors_api_key()
	coordinates<-list(c(31.7128488,-0.3742984),c(31.4098558,-1.0151675))
	...
	tmp	<- as.matrix(subset(coords, select=c(Longitude,Latitude)))		
	df	<- rtr[, {
				list(	TR_SPHD_HWY= 1e-3 * min( distHaversine( c(TR_LON, TR_LAT), tmp ) ),
						REC_SPHD_HWY=1e-3 * min( distHaversine( c(REC_LONG, REC_LAT), tmp ) )
				)				
			}, by=c('PAIRID','TR_COMM_NUM_A_MIG','REC_COMM_NUM_A')]
	highway.corridor.radius	<- 5
	df[, TR_TARGETCAT:= as.character(factor(TR_SPHD_HWY<=highway.corridor.radius, levels=c(TRUE,FALSE), labels=c('highway','other')))]
	df[, REC_TARGETCAT:= as.character(factor(REC_SPHD_HWY<=highway.corridor.radius, levels=c(TRUE,FALSE), labels=c('highway','other')))]
	set(df, df[, which(grepl('emig',TR_COMM_NUM_A_MIG))], 'TR_TARGETCAT', 'external')
	set(df, df[, which(grepl('umig',TR_COMM_NUM_A_MIG))], 'TR_TARGETCAT', 'unknown')
	
	df[, table(TR_TARGETCAT,REC_TARGETCAT)]
	#	--> equal in/out, suggesting hub
	
}

RakaiFull.phylogeography.190327.highway.distancesToHighway<- function()
{
	if(0)
	{
	...
	# calculate the great circle distance between communities (comgps)
	# and points on the highway (coords)
	...
	load(infile.inference.data)
	tmp	<- as.matrix(subset(coords, select=c(Longitude,Latitude)))
	df	<- subset(rtr3, !grepl('umig|emig',TR_COMM_NUM_A_MIG_2YR))	
	df	<- df[, {
				list(	TR_SPHD_HWY= 1e-3 * min( distHaversine( c(TR_LON_MIG_2YR, TR_LAT_MIG_2YR), tmp ) ),
						REC_SPHD_HWY=1e-3 * min( distHaversine( c(REC_LONG, REC_LAT), tmp ) )
						)				
			}, by=c('PAIRID','TR_COMM_NUM_A_MIG_2YR','REC_COMM_NUM_A')]
	
	ggplot(df, aes(x=TR_SPHD_HWY, y=REC_SPHD_HWY)) + 
			geom_abline(slope=1, lty=2) +
			geom_jitter(width=1, height=1) +
			theme_bw()
	
	tmp	<- melt(df, id.vars=c('PAIRID','TR_COMM_NUM_A_MIG_2YR','REC_COMM_NUM_A'))
	ggplot(tmp, aes(x= value, fill=variable)) +
			geom_histogram(binwidth=1, alpha=0.5, position='identity') +
			theme_bw() 
	
	tmp	<- copy(df)
	tmp[, DIST_DIFF:= TR_SPHD_HWY-REC_SPHD_HWY]
	tmp[, DIST_DIFF_C:= cut(TR_SPHD_HWY-REC_SPHD_HWY, breaks=c(-Inf,-0.2,0.2,Inf), labels=c('TR_CLOSER_TO_HGHWAY','SAME','REC_CLOSER_TO_HGHWAY'))]
	
	ggplot(tmp, aes(x=DIST_DIFF)) + geom_histogram(binwidth=1)
		
	tmp[, table(DIST_DIFF_C)]
	tmp2	<- subset(tmp,DIST_DIFF_C!='SAME')
	ggplot(tmp2, aes(x=TR_SPHD_HWY, y=REC_SPHD_HWY)) + 
			geom_abline(slope=1, lty=2) +
			geom_jitter(width=.1, height=.1) +
			theme_bw()
	
	tmp3	<- melt(tmp2, id.vars=c('PAIRID','TR_COMM_NUM_A_MIG_2YR','REC_COMM_NUM_A'), measure.vars=c('TR_SPHD_HWY','REC_SPHD_HWY'))
	ggplot(tmp3, aes(x= value, fill=variable)) +
			geom_histogram(binwidth=1, alpha=0.5, position='identity') +
			theme_bw() 
...
{	
	...
	require(ggplot2)
	indir	<- "~/Dropbox (SPH Imperial College)/Rakai Fish Analysis/full_run"
	infiles	<- list.files(indir, pattern='RakaiAll_output_190327.*')
	infiles	<- data.table(FI=infiles[grepl('prAreas',infiles) & grepl('flowsetc.csv',infiles)])
	...
	infiles[, OPT_MIG:= paste0(substring(OPT,3,4), ' months')]
	infiles[, OPT_UNKMIG:= factor(substring(OPT,5,5), levels=c('0','1'), labels=c('fishing communities','inland communities'))]
	infiles[, OPT_HIVCNT:= factor(substring(OPT,7,7), levels=c('1','0'), labels=c('hivcount','popcount'))]
	infiles[, OPT_INFL:= as.numeric(substring(OPT,8,9))/10]	
	infiles[, STRAT:= factor(grepl('Gender',FI), levels=c(FALSE,TRUE), labels=c('overall','by gender'))]
	...
	tmp	<- subset(se, STAT=='flows' & STRAT=='overall')
	ggplot(tmp, aes(x=TRM_CAT, y=M, ymin=CL, ymax=CU, colour=interaction(OPT_HIVCNT,OPT_INFL))) +
			geom_point(position=position_dodge(0.8)) +
			geom_errorbar(position=position_dodge(0.8))
	
	subset(se, STAT=='flows' & STRAT=='overall' & TRM_CAT=='inland -> inland')
}
...
{	
	...
	str(mc[[1]])
	gc()	
	mcmc.file	<- paste0(outfile.base,'_prAreas.rda')
	...
	pars[, REC_TARGETCAT:= gsub('^([a-z]+)\\:([MF])$','\\1',REC_TRM_CAT)]
	pars	<- pars[, list(VALUE=sum(VALUE)), by=c('TR_TARGETCAT','REC_TARGETCAT','VARIABLE','SAMPLE')]
	write.csv(pars, row.names=FALSE, file=gsub('\\.rda','PI2.csv',mcmc.file))
	control		<- list(	quantiles= c('CL'=0.025,'IL'=0.25,'M'=0.5,'IU'=0.75,'CU'=0.975),
			flowratios= list( c('inland/fishing', 'inland fishing', 'fishing inland')),
			outfile=gsub('\\.csv','_flowsetc.csv',gsub('\\.rda','_PI2.csv',mcmc.file)))
	source.attribution.aggmcmc.getKeyQuantities(gsub('\\.rda','_PI2.csv',mcmc.file), control)	
}
...
RakaiFull.phylogeography.190327.data.eligibility.participation.sequenced<- function()
{
	require(data.table)
	...
	infile.migrant2		<- "~/Dropbox (SPH Imperial College)/Rakai Pangea Meta Data/Data for Fish Analysis Working Group/migrants_withMissingGPS.csv"
	inmigrant2			<- as.data.table(read.csv(infile.migrant2))
	inmigrant2			<- subset(inmigrant2, !is.na(inmig_lon), select=c(RCCS_studyid, visit, inmig_lon, inmig_lat, inmig_place, inmig_admin0, inmig_admin1, inmig_admin2, inmig_admin3, inmig_admin4))
	setnames(inmigrant2, c('RCCS_studyid','visit'), c('RID','VISIT'))
	set(inmigrant2, inmigrant2[, which(grepl('E095382|D110171',RID))], 'source_class', 'fish')
	set(inmigrant2, inmigrant2[, which(grepl('J189465|C020384|F097032|A078484|H091385',RID))], 'source_class', 'inland')
	set(inmigrant2, inmigrant2[, which(grepl('G078375',RID))], 'source_class', 'external')
	
	infile.migrant		<- "~/Dropbox (SPH Imperial College)/Rakai Pangea Meta Data/Data for Fish Analysis Working Group/all_migrant_250319.rda"
	load(infile.migrant)
	...
	set(inmigrant, inmigrant[, which(is.na(source_class))], 'source_class', 'unknown')
	set(inmigrant, NULL, 'source_class', inmigrant[, gsub('fishing','fish',tolower(source_class))])
	setnames(inmigrant, colnames(inmigrant), gsub('RCCS_STUDYID','RID',toupper(colnames(inmigrant))))
	
	inmigrant			<- merge(inmigrant,inmigrant2,by=c('RID','VISIT'),all.x=TRUE)
	tmp					<- inmigrant[, which(!is.na(inmig_lon) & is.na(INMIG_LON))]
	for(x in c('inmig_lon','inmig_lat','inmig_place','inmig_admin0','inmig_admin1','inmig_admin2','inmig_admin3','inmig_admin4','source_class'))
		set(inmigrant, tmp, toupper(x), inmigrant[[x]][tmp])
	set(inmigrant, NULL, c('inmig_lon','inmig_lat','inmig_place','inmig_admin0','inmig_admin1','inmig_admin2','inmig_admin3','inmig_admin4','source_class'), NULL)
	set(inmigrant, NULL, 'DATE', inmigrant[, hivc.db.Date2numeric(DATE)])
	setnames(inmigrant, c('DATE','INMIG_LOC','SOURCE_CLASS'), c('INMIGRATE_DATE','INMIG_LOCATION','INMIG_LOC')) #for compatibility with olli s code		
	#
	#	inmigrants done
	#
	...
	save(des, desa, desm, rds, de, rd, di, inmigrant, file='~/Dropbox (SPH Imperial College)/Rakai Fish Analysis/190327_sampling_by_gender_age.rda')
}

RakaiFull.phylogeography.190327.determine.sampling.categories.required<- function()
{
	indir	<- "~/Dropbox (SPH Imperial College)/Rakai Fish Analysis/full_run"
	infiles	<- data.table(	F=	c(	  "RakaiAll_output_190327_w250_s20_p25_d50_stagetwo_rerun23_min30_conf60_phylogeography_data_with_inmigrants.rda"
									, "RakaiAll_output_190327_w250_s20_p25_d50_stagetwo_rerun23_min30_conf65_phylogeography_data_with_inmigrants.rda"
									, "RakaiAll_output_190327_w250_s20_p25_d50_stagetwo_rerun23_min30_conf70_phylogeography_data_with_inmigrants.rda"
									, "RakaiAll_output_190327_w250_s20_p25_d50_stagetwo_rerun23_min30_conf50_phylogeography_data_with_inmigrants.rda"
									, "RakaiAll_output_190327_w250_s20_p25_d50_stagetwo_rerun23_min30_conf55_phylogeography_data_with_inmigrants.rda"
						   	...
						   	TR= 'TR_INMIGRATE_2YR',
						   	REC= 'REC_INMIGRATE_2YR',
							MIG= 'TR_COMM_NUM_A_MIG_2YR')
	tmp		<- data.table(	F="RakaiAll_output_190327_w250_s20_p25_d50_stagetwo_rerun23_min30_conf60_phylogeography_data_with_inmigrants.rda",
							TR= c('TR_INMIGRATE_05YR','TR_INMIGRATE_1YR','TR_INMIGRATE_3YR','TR_INMIGRATE_4YR'),
							REC= c('REC_INMIGRATE_05YR','REC_INMIGRATE_1YR','REC_INMIGRATE_3YR','REC_INMIGRATE_4YR'),
							MIG= c('TR_COMM_NUM_A_MIG_05YR','TR_COMM_NUM_A_MIG_1YR','TR_COMM_NUM_A_MIG_3YR','TR_COMM_NUM_A_MIG_4YR'))			
	...
	for(i in seq_len(nrow(infiles)))
	{
		...
		load(file.path(indir,infiles[i,F]))
		
		rtr	<- copy(rtr3)
		setnames(rtr, infiles[i,TR], 'TR_INMIGRATE')
		setnames(rtr, infiles[i,REC], 'REC_INMIGRATE')
		setnames(rtr, infiles[i,MIG], 'TR_COMM_NUM_A_MIG')
		rtr	<- subset(rtr, select=c(	'PAIRID','TR_RID','TR_COMM_NUM','TR_COMM_NUM_A','TR_COMM_NUM_A_MIG',
						'TR_SEX','TR_BIRTHDATE','TR_COMM_TYPE','TR_INMIG_LOC','TR_INMIGRATE',
						'REC_RID','REC_COMM_NUM','REC_COMM_NUM_A',
						'REC_SEX','REC_BIRTHDATE','REC_COMM_TYPE','REC_INMIGRATE'))		
...
RakaiFull.phylogeography.190327.flows.wrapper<- function()
{
	require(big.phylo)
	#
	#	make data set
...
RakaiFull.phylogeography.190327.figure.migration.pie<- function()
{
	library(data.table)
	library(scatterpie)
	library(ggplot2)
	...
	infile.anymisedcom	<- '~/Dropbox (SPH Imperial College)/Rakai Pangea Meta Data/Data for Fish Analysis Working Group/PANGEA_Rakai_community_anonymized_IDs.csv'
	infile				<- "~/Dropbox (SPH Imperial College)/Rakai Fish Analysis/full_run/RakaiAll_output_190327_w250_s20_p25_d50_stagetwo_rerun23_min30_conf60_phylogeography_data_with_inmigrants.rda"
	outfile				<- "~/Dropbox (SPH Imperial College)/Rakai Fish Analysis/full_run/RCCS_inmigrants_piechart.pdf"
	load(infile)
	
	#	get migration counts
	df		<- desm[, list(HIV_1516_YES=sum(HIV_1516_YES)), by=c('COMM_NUM','COMM_NUM_A','INMIG_LOC')]
	df		<- dcast.data.table(df, COMM_NUM+COMM_NUM_A~INMIG_LOC, value.var='HIV_1516_YES')
	setnames(df, 'fish', 'fisherfolk')
	for(x in c('external','fisherfolk','inland','resident','unknown'))
	{
		set(df, which(is.na(df[[x]])), x, 0)		
	}		
	set(df, NULL, 'COMM_NUM', df[, as.character(COMM_NUM)])
	set(df, NULL, 'resident', df[, as.double(resident)])
	set(df, NULL, 'unknown', df[, as.double(unknown)])
	set(df, NULL, 'inland', df[, as.double(inland)])
	set(df, NULL, 'fisherfolk', df[, as.double(fisherfolk)])
	set(df, NULL, 'external', df[, as.double(external)])
	df[, INMIGRANT:= external + fisherfolk + inland + unknown]
	df[, HIV:= external + fisherfolk + inland + resident + unknown]
	# 	add long/lat
	zc		<- as.data.table(read.csv(infile.anymisedcom, stringsAsFactors=FALSE))
	tmp		<- unique(subset(zc, select=c(COMM_NUM, longitude, latitude)), by='COMM_NUM')
	df		<- merge(df, tmp, by='COMM_NUM', all.x=TRUE)
	...
	#ggplot(df) + geom_point(aes(x=LONGITUDE, y=LATITUDE))
	
	ggmap(zm) +
			geom_scatterpie(aes(x=LONGITUDE, y=LATITUDE, group=COMM_NUM, r=R), 
							data=df,
							cols=c('RESIDENT','INMIGRANT')) +
			scale_fill_manual(values=c('RESIDENT'='grey50','INMIGRANT'='darkorange')) + 			
			geom_scatterpie_legend(df$R, x=31.35, y=-0.4, n=4, labeller=function(radius) round((radius*300)^3))
			
	ggsave(file=outfile, w=7, h=5)
	
	df	<- subset(rtr3, select=c(TR_RID, TR_COMM_TYPE, TR_INMIGRATE_2YR))
	setnames(df, colnames(df), gsub('^TR_','',colnames(df)))	
	tmp	<- subset(rtr3, select=c(REC_RID, REC_COMM_TYPE, REC_INMIGRATE_2YR))
	setnames(tmp, colnames(tmp), gsub('^REC_','',colnames(tmp)))
	df	<- rbind(df, tmp)	
	df	<- unique(df)
	set(df, df[, which(COMM_TYPE!='fisherfolk')], 'COMM_TYPE', 'inland')
	df[, table(INMIGRATE_2YR, COMM_TYPE)]
}

RakaiFull.phylogeography.190327.figure.traces<- function()
{
	require(bayesplot)
	...
	outfile.base<- gsub('\\.rda','',mcmc.file)
	load(mcmc.file)
	pars			<- mc$pars$PI[,c(1,124)]
	colnames(pars)	<- c('PI-1','PI-124')
	burnin.n		<- 0.05 * nrow(pars)
	p				<- mcmc_trace(pars, pars=colnames(pars), facet_args = list(ncol = 1), n_warmup=burnin.n) +
						coord_cartesian(xlim=c(0,1e5))
	pdf(file=paste0(outfile.base,'_marginaltraces_suppfigure.pdf'), w=12, h=6)
	print(p)
	dev.off()	
	
	mcmc.file	<- '~/Dropbox (SPH Imperial College)/Rakai Fish Analysis/full_run/RakaiAll_output_181006_w250_s20_p25_d50_stagetwo_rerun23_min30_conf60_phylogeography_samcmc190327_nsweep1e5_opt112401_prAreas.rda'
	outfile.base<- gsub('\\.rda','',mcmc.file)
	load(mcmc.file)
	pars			<- mc$pars$PI[,2,drop=FALSE]
	colnames(pars)	<- c('PI*-2')
	burnin.n		<- 0
	p				<- mcmc_trace(pars, pars=colnames(pars), facet_args = list(ncol = 1), n_warmup=burnin.n) +
			coord_cartesian(xlim=c(0,1e5))
	pdf(file=paste0(outfile.base,'_marginaltraces_suppfigure.pdf'), w=12, h=3)
	print(p)
	dev.off()	
	
}

RakaiFull.phylogeography.190327.figure.areas.on.map<- function()
{
	require(data.table)
	require(scales)
	require(ggplot2)
	...
	tmp	<- unique(subset(desm, select=c(COMM_NUM, COMM_NUM_A, LONG, LAT, COMM_TYPE)))
	#	replace 16m with 16 and 107
	tmp	<- rbind( 	subset(tmp, COMM_NUM!='16m'),
					data.table(COMM_NUM=c('16','107'), COMM_NUM_A=c('thd','thx'), LONG=c(31.49650, 31.52377), LAT=c(-0.8339917,-0.8341833), COMM_TYPE='inland'))
	
	load(infile.subdistricts)
	rasdata	<- as.data.table(rasdata)
	rasdata	<- subset(rasdata, select=c(latitude, longitude, lakedis, pop_class, loc))
	   
	colors	<- c(	'Fishing'='chocolate3', 'fisherfolk'='chocolate3',
					'Inland'='#66BD63', 'inland'='#66BD63',
					'External'='deepskyblue1')
		
	ggmap(zm) + 
			geom_raster(data=rasdata, aes(x=longitude, y=latitude, fill=pop_class), alpha=0.4) +
			geom_point(data=tmp, aes(x=LONG, y=LAT, fill=COMM_TYPE), shape=21, size=6, alpha=1, colour='black') +
			scale_fill_manual(values=colors) +
			coord_cartesian() + 
			guides(fill='none')
	ggsave(file=paste0(outfile.base,'/190327_RCCS_areamap.pdf'), w=6, h=6, useDingbats=FALSE)
}

RakaiFull.phylogeography.190327.figure.transmissionflows.on.map<- function()
{
	require(data.table)
	require(scales)
	require(ggplot2)
	...
	mig.colors	<- c(	'fishcomm'='chocolate3','inlandcomm'='#66BD63', 						
						'resident_fish'="chocolate4", 'inmigrant_from_fisherfolk'="chocolate4",
						'resident_inland'="#00441B", 'inmigrant_from_inland'="#00441B",
						# 'resident_inland'="#66BD63",						
						'inmigrant_from_unknownloc'=NA, 
						'inmigrant_from_external'='deepskyblue1')
	#
	#	flows inland -> fishing
	#
	ggmap(zm) +	
			geom_point( data=subset(comm.locs, COMM_TYPE=='fisherfolk', c(LONG, LAT)), aes(x=LONG, y=LAT), colour=mig.colors['fishcomm'], size=4 ) +
			geom_point( data=subset(comm.locs, COMM_TYPE=='inland', c(LONG, LAT)), aes(x=LONG, y=LAT), colour=mig.colors['inlandcomm'], size=4 ) +
			geom_point( data=unique(subset(dc, substr(REC_COMM_NUM_A,1,1)=='f' & grepl('inmigrant_from_inland',TR_INMIGRATE), c(TR_COMM_NUM_A, TR_X, TR_Y) )),
					aes(x=TR_X, y=TR_Y), colour=mig.colors['inlandcomm'], size=4, pch=18) +
			geom_curve(	data=subset(dc, !substr(TR_COMM_NUM_A,1,1)%in%c('f','e') & substr(REC_COMM_NUM_A,1,1)=='f' & FLOW>0), 
					aes(x=TR_X_EDGE, xend=REC_X_EDGE, y=TR_Y_EDGE, yend=REC_Y_EDGE, colour=TR_INMIGRATE, size=EDGE_LABEL2), 
					curvature=curvature, arrow=arrow, lineend="butt", linejoin='mitre') +
			#geom_text(	data=subset(dc, substr(TR_COMM_NUM_A,1,1)!='f' & substr(REC_COMM_NUM_A,1,1)=='f' & FLOW>0), 
			...
			#		size=label.size) +
			coord_cartesian() +
			scale_colour_manual(values=mig.colors) +			
			#scale_size(breaks=1:10, range=c(0.5,2)) +
			scale_size_identity() +
			labs(x='', y='') +
			theme(legend.position='bottom', legend.box = "vertical") + 
	...
	ggmap(zm) +	
			...
			geom_point( data=subset(comm.locs, COMM_TYPE=='inland', c(LONG, LAT)), aes(x=LONG, y=LAT), colour=mig.colors['inlandcomm'], size=4 ) +
			geom_point( data=unique(subset(dc, substr(REC_COMM_NUM_A,1,1)!='f' & grepl('inmigrant_from_fisherfolk',TR_INMIGRATE), c(TR_COMM_NUM_A, TR_X, TR_Y) )),
					aes(x=TR_X, y=TR_Y), colour=mig.colors['fishcomm'], size=4, pch=18) +
...
RakaiFull.phylogeography.190327.figure.prevalence.gender<- function()
{
	require(data.table)
	require(scales)
	require(ggplot2)
	...
	set(comgps, NULL, 'COMM_NUM', comgps[, as.integer(as.character(COMM_NUM))])
	df	<- merge(df, comgps, by='COMM_NUM', all.x=TRUE)
	#	Kate don t merge 107 and 16 to 16m
	...
	tmp	<- as.data.table(read.csv('~/Dropbox (SPH Imperial College)/Rakai Pangea Meta Data/Data for Fish Analysis Working Group/PANGEA_Rakai_community_anonymized_IDs.csv', stringsAsFactors=FALSE))
	tmp	<- unique(subset(tmp, select=c(COMM_NUM, COMM_NUM_A, COMM_TYPE)))
	set(tmp, tmp[,which(COMM_NUM=='16m')],'COMM_NUM','16')
	tmp	<- rbind(tmp, data.table(COMM_NUM='107', COMM_NUM_A='thx',  COMM_TYPE='trading'))	
	df	<- merge(df, tmp, by='COMM_NUM', all.x=TRUE)
	...
	tmp		<- subset(dgg, variable!='RFM')
	set(tmp, tmp[, which(COMM_TYPE!='fisherfolk')], 'COMM_TYPE', 'inland')
	set(tmp, tmp[, which(COMM_TYPE=='fisherfolk')], 'COMM_TYPE', 'fishing')
	set(tmp, NULL, 'COMM_TYPE', tmp[, factor(COMM_TYPE, levels=c('inland','fishing'))])
	set(tmp, tmp[, which(variable=='PM')], 'variable', 'men')
	set(tmp, tmp[, which(variable=='PF')], 'variable', 'women')
	tmp2	<- subset(tmp, variable=='men', c(COMM_NUM, COMM_NUM_A, COMM_TYPE, M))	
	tmp2	<- tmp2[order(COMM_TYPE, M),]
...
RakaiFull.phylogeography.190327.figure.flows<- function()
{
	...
	z		<- as.data.table(read.csv("~/Dropbox (SPH Imperial College)/Rakai Fish Analysis/full_run/RakaiAll_output_190327_w250_s20_p25_d50_stagetwo_rerun23_min30_conf60_phylogeography_samcmc190327_nsweep1e5_opt112401020_prAreas_PIGender.csv"))
	z[, TR_LOC:= gsub('^([a-zA-Z]+)\\:([a-zA-Z]+)','\\1', TR_TARGETCAT)]
	z[, REC_LOC:= gsub('^([a-zA-Z]+)\\:([a-zA-Z]+)','\\1', REC_TARGETCAT)]
	z[, FLOW:= paste0(TR_LOC,' -> ',REC_LOC)]
	set(z, z[, which(TR_LOC=='inland' & REC_LOC=='inland')], 'FLOW', 'within')
	set(z, z[, which(TR_LOC=='fishing' & REC_LOC=='fishing')], 'FLOW', 'within')	
	z			<- z[, list(VALUE=sum(VALUE)), by=c('FLOW','SAMPLE')]
	z			<- z[, list(P=names(quantiles), Q=unname(quantile(VALUE, p=quantiles))), by=c('FLOW')]
	z			<- dcast.data.table(z, FLOW~P, value.var='Q')
	set(z, NULL, 'FLOW', z[, factor(FLOW, levels=rev(c('within','inland -> fishing','fishing -> inland','external -> fishing','external -> inland')))])
	...
	ggplot(z) +
			...
			theme_classic() +
			scale_y_continuous(labels=scales:::percent, lim=c(0,1), expand=c(0,0)) +
			labs(y='\npredicted transmission flows', x='') +
			coord_flip() 			
	ggsave(file='~/Dropbox (SPH Imperial College)/Rakai Fish Analysis/full_run/RakaiAll_output_190327_w250_s20_p25_d50_stagetwo_rerun23_min30_conf60_phylogeography_samcmc190327_nsweep1e5_opt112401020_prAreas_flows.pdf', w=5, h=2, useDingbats=FALSE)	
...
RakaiFull.phylogeography.190327.figure.gender.and.migration.map<- function()
{
	library(data.table)
	library(ggmap)
	library(ggplot2)
	...
	infile.inference	<- "~/Dropbox (SPH Imperial College)/Rakai Fish Analysis/full_run/RakaiAll_output_190327_w250_s20_p25_d50_stagetwo_rerun23_min30_conf60_phylogeography_data_with_inmigrants.rda"	
	load(infile.inference)
	df	<- subset(rtr3, select=c(REC_COMM_NUM_A, REC_LONG, REC_LAT, REC_COMM_TYPE, TR_SEX, TR_INMIGRATE_2YR))
	set(df, df[, which(grepl('inmigrant',TR_INMIGRATE_2YR))], 'TR_INMIGRATE_2YR', 'inmigrant')
	df	<- df[, list( TR_N= length(TR_SEX), TR_M=length(which(TR_SEX=='M')), TR_F=length(which(TR_SEX=='F'))), by=c('REC_COMM_NUM_A','REC_LONG','REC_LAT','REC_COMM_TYPE','TR_INMIGRATE_2YR')]
	df[, R:= TR_N/1000]
	df[, R:= TR_N^(1/3)/100]
	ggmap(zm) +
			geom_scatterpie(aes(x=REC_LONG, y=REC_LAT, group=REC_COMM_NUM_A, r=R), 
					data=subset(df, TR_INMIGRATE_2YR=='resident'),
					cols=c('TR_M','TR_F')) +
			scale_fill_manual(values=c('TR_M'='deepskyblue','TR_F'='hotpink2')) + 			
			geom_scatterpie_legend(df$R, x=31.35, y=-0.4, n=4, labeller=function(radius) round((radius*100)^3))
	
	ggmap(zm) +
			geom_scatterpie(aes(x=REC_LONG, y=REC_LAT, group=REC_COMM_NUM_A, r=R), 
					data=subset(df, TR_INMIGRATE_2YR=='inmigrant'),
					cols=c('TR_M','TR_F')) +
			scale_fill_manual(values=c('TR_M'='deepskyblue','TR_F'='hotpink2')) + 			
			geom_scatterpie_legend(df$R, x=31.35, y=-0.4, n=4, labeller=function(radius) round((radius*100)^3))
}
...
RakaiFull.phylogeography.190327.figure.gender.and.migration.sources<- function()
{
	infile.inference	<- "~/Dropbox (SPH Imperial College)/Rakai Fish Analysis/full_run/RakaiAll_output_190327_w250_s20_p25_d50_stagetwo_rerun23_min30_conf60_phylogeography_data_with_inmigrants.rda"
	mcmc.file			<- '~/Dropbox (SPH Imperial College)/Rakai Fish Analysis/full_run/RakaiAll_output_190327_w250_s20_p25_d50_stagetwo_rerun23_min30_conf60_phylogeography_samcmc190327_nsweep1e5_opt112401.rda'
...
RakaiFull.phylogeography.190327.figure.migration.sources<- function()
{
	...
	#	aggregate MCMC output to sources by migration status
	#	compare prop of trms from migrants to prop of inf who are migrants, etc
	daggregateTo	<- subset(dobs, select=c(TRM_CAT_PAIR_ID, TR_TRM_CATEGORY, REC_TRM_CATEGORY))
	daggregateTo[, TR_TARGETCAT:= gsub('^.*:([0-1])$','\\1',daggregateTo$TR_TRM_CATEGORY)]
	daggregateTo[, REC_TARGETCAT:= 'Any']
	set(daggregateTo, NULL, c('TR_TRM_CATEGORY','REC_TRM_CATEGORY'), NULL)	
	control		<- list(	burnin.p=0.05, 
			thin=NA_integer_, 
	...
	#    INMIG HIV_1516_YES HIV_1516_NO
	#1:     1         1365        5146
	#2:     0         3777       15594
	# 	1365/5142 = 0.2654609
	qbeta(p=c(0.025,0.5,0.975), shape1=6511+1, shape2=25882-6511+1)
	#   0.2463161 0.2515776 0.2568882
	qbeta(p=c(0.025,0.5,0.975), shape1=1365+1, shape2=5142-1365+1)
	#	0.2535718 0.2655217 0.2777050
	
	z	<- subset(mca, TR_TARGETCAT==1)
	z[, PROP_MIG_OF_POP:= rbeta(nrow(z), shape1=6511+1, shape2=25882-6511+1)]
	z[, PROP_MIG_OF_INF:= rbeta(nrow(z), shape1=1365+1, shape2=5142-1365+1)]
	setnames(z, 'VALUE', 'PROP_MIG_OF_SRCS')
	tmp	<- melt(z, id.vars=c('SAMPLE'),measure.vars=c('PROP_MIG_OF_SRCS','PROP_MIG_OF_POP','PROP_MIG_OF_INF'))
	tmp	<- tmp[, list(Q= quantile(value, p=c(0.025,0.25,0.5,0.75,0.975)), P=c('CL','IL','M','IU','CU')), by='variable']
	tmp	<- dcast.data.table(tmp, variable~P, value.var='Q')
	set(tmp, NULL, 'variable', tmp[, factor(variable, levels=c('PROP_MIG_OF_SRCS','PROP_MIG_OF_INF','PROP_MIG_OF_POP'), labels=c('among phylogenetically\nlikely transmitters','among infected participants','among participants'))])
	ggplot(tmp, aes(x=variable, middle=M, lower=IL, upper=IU, min=CL, max=CU)) +
			geom_boxplot(stat='identity', fill='grey50') +
			theme_bw() +
			scale_y_continuous(labels=scales:::percent, lim=c(0,0.5), expand=c(0,0)) +
			guides(fill='none') +
			labs(x='', y='proportion of in-migrants\n') +
			theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1),
					panel.grid.major.x=element_blank(), 
					panel.grid.minor.x=element_blank()) 
	...
	mca			<- source.attribution.mcmc.aggregateToTarget(mcmc.file=mcmc.file, daggregateTo=daggregateTo, control=control)	
	z			<- copy(mca)
	z			<- z[, list(TR_TARGETCAT=TR_TARGETCAT, VALUE=VALUE/sum(VALUE)), by=c('REC_TARGETCAT','SAMPLE')]
	z[, TR_MIGRANT:= TR_TARGETCAT]
	z[, TR_GENDER:= 'Any']		
	z			<- z[, list(P=names(quantiles), Q=unname(quantile(VALUE, p=quantiles))), by=c('REC_TARGETCAT','TR_MIGRANT','TR_GENDER')]
	z			<- dcast.data.table(z, REC_TARGETCAT+TR_MIGRANT+TR_GENDER~P, value.var='Q')
	...
	ggplot(tmp) +
			geom_boxplot(aes(x=REC_TARGETCAT, ymin=CL, ymax=CU, lower=IL, upper=IU, middle=M, fill=TR_GENDER), stat='identity', width=0.7, fill='grey50') +
			theme_bw() +
			scale_y_continuous(labels=scales:::percent, lim=c(0,0.5), expand=c(0,0)) +
			labs(x='\nlocation', y='proportion of transmissions from in-migrants\n') +
			theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1),
					panel.grid.major.x=element_blank(), 
					panel.grid.minor.x=element_blank()) 
	...
	ggplot(subset(z, !grepl('outside',FLOW))) +
			...
			theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1),
					panel.grid.major.x=element_blank(), 
					panel.grid.minor.x=element_blank())			
	ggsave(file=gsub('\\.rda','_sourcesFromImmigrants.pdf',mcmc.file), w=6, h=6, useDingbats=FALSE)
	#                                                                     FLOW TR_MIGRANT TR_GENDER         CL        CU         IL         IU          M
	#1: sources of transmissions\nbetween RCCS inland and\nfishing communities   resident       Any 0.30631273 0.6496422 0.41497416 0.53630222 0.47489995
	#2: sources of transmissions\nbetween RCCS inland and\nfishing communities in-migrant       Any 0.35035781 0.6936873 0.46369778 0.58502584 0.52510005
	#3:                            sources of transmissions\nfrom outside RCCS in-migrant       Any 1.00000000 1.0000000 1.00000000 1.00000000 1.00000000
	#4:   sources of transmissions\nwithin RCCS inland or\nfishing communities   resident       Any 0.88396698 0.9577513 0.91351208 0.93898474 0.92698015
	#5:   sources of transmissions\nwithin RCCS inland or\nfishing communities in-migrant       Any 0.04224874 0.1160330 0.06101526 0.08648792 0.07301985
}
...
RakaiFull.phylogeography.190327.figure.gender.and.migration.flows<- function()
{
	...
	z			<- copy(mca)
	z[, FLOW:= paste0(gsub('^([a-z]+)\\:.*','\\1',TR_TARGETCAT),' -> ',gsub('^([a-z]+)\\:.*','\\1',REC_TARGETCAT))]
	z[, TR_GENDER:= gsub('^[a-z]+:([FM]):?.*','\\1',TR_TARGETCAT)]
	z[, TR_MIGRANT:= 'Any']
	z			<- z[, list(TR_GENDER=TR_GENDER, VALUE=VALUE/sum(VALUE)), by=c('FLOW','TR_MIGRANT','SAMPLE')]	
	z			<- z[, list(P=names(quantiles), Q=unname(quantile(VALUE, p=quantiles))), by=c('FLOW','TR_MIGRANT','TR_GENDER')]
	z			<- dcast.data.table(z, FLOW+TR_MIGRANT+TR_GENDER~P, value.var='Q')
	set(z, NULL, 'FLOW', z[, factor(FLOW, levels=rev(c('fishing -> fishing','fishing -> inland', 'inland -> fishing','inland -> inland','external -> fishing','external -> inland')))])
	ans			<- copy(z)
	ggplot(subset(z, TR_GENDER=='M')) +
			geom_hline(yintercept=0.5, lty=2) +
			geom_boxplot(aes(x=FLOW, ymin=CL, ymax=CU, lower=IL, upper=IU, middle=M, fill=TR_GENDER), stat='identity') +
			theme_bw() +
			coord_flip() +
			scale_y_continuous(labels=scales:::percent, lim=c(0,1)) +
			scale_fill_manual(values=c('M'='deepskyblue', 'F'='hotpink2')) +
			labs(x='estimated transmission flows\n', y='\nproportion of transmissions from men') +
			guides(fill='none')
	ggsave(file=gsub('\\.rda','_flowsFromMen.pdf',mcmc.file), w=5, h=5, useDingbats=FALSE)
	
	set(z, NULL, 'FLOW', z[, factor(FLOW, levels=c('fishing -> fishing','fishing -> inland', 'inland -> fishing','inland -> inland','external -> fishing','external -> inland'))])
	ggplot(z) +
			geom_hline(yintercept=0.5, lty=2) +
			geom_boxplot(aes(x=TR_GENDER, ymin=CL, ymax=CU, lower=IL, upper=IU, middle=M, fill=TR_GENDER), stat='identity') +
			theme_bw() +
			coord_flip() +
			scale_y_continuous(labels=scales:::percent, lim=c(0,1)) +
			scale_fill_manual(values=c('M'='deepskyblue', 'F'='hotpink2')) +
	...
	ggsave(file=gsub('\\.rda','_flowsFromMen2.pdf',mcmc.file), w=6, h=6, useDingbats=FALSE)
	
	#	aggregate MCMC output to fish<->inland by migration status of transmitter
	daggregateTo	<- subset(dobs, select=c(TRM_CAT_PAIR_ID, TR_TRM_CATEGORY, REC_TRM_CATEGORY))
	daggregateTo[, TR_TARGETCAT:= paste0(	gsub('^e$','external',gsub('^f$','fishing',gsub('^a|i|t$','inland',substring(daggregateTo$TR_TRM_CATEGORY,1,1)))),
					':',
					gsub('.*:([0-9])$','\\1',daggregateTo$TR_TRM_CATEGORY)
	...
	ggplot(z) +
			...
			coord_flip() +
			scale_y_continuous(labels=scales:::percent,  lim=c(0,1)) +
			labs(x='estimated transmission flows\n', y='\nproportion of transmissions from immigrants')
	ggsave(file=gsub('\\.rda','_flowsFromImmigrants.pdf',mcmc.file), w=5, h=5, useDingbats=FALSE)
...
RakaiFull.phylogeography.190327.migration.datasets<- function()
{
	...
	load(infile)	
	deold	<- copy(de)
	deold	<- subset(deold, select=c(RID, PERM_ID, VISIT, INMIG_2YRS_LOC))
	setnames(deold, 'INMIG_2YRS_LOC', 'INMIG_2YRS_LOC_OLD')
	
	infile		<- '~/Dropbox (SPH Imperial College)/Rakai Fish Analysis/190327_sampling_by_gender_age.rda'
	load(infile)	
	de	<- subset(de, select=c(RID, PERM_ID, VISIT, INMIG_2YRS_LOC))
	
	de	<- merge(de, deold, by= c('RID','PERM_ID','VISIT'))
	de[, table(INMIG_2YRS_LOC, INMIG_2YRS_LOC_OLD)]
	#	             INMIG_2YRS_LOC_OLD
	#INMIG_2YRS_LOC external fisherfolk inland resident unknown
    #external        360          0      0     1719       0
    #fisherfolk        0          8      0       18       0
    #inland            0          0    541     2712       0
    #resident         20          1     48    31057       8
    #unknown           1          1      6      938     207
	
}

RakaiFull.phylogeography.190327.figure.migration.numbers<- function()
{
	require(data.table)
	infile		<- '~/Dropbox (SPH Imperial College)/Rakai Fish Analysis/190327_sampling_by_gender_age.rda'
	outfile.base<- '~/Dropbox (SPH Imperial College)/Rakai Fish Analysis/full_run/190327_'
	load(infile)

	tmp			<- desm[, list(PART_EVER=sum(PART_EVER)), by=c('COMM_TYPE','INMIG_2YRS_LOC')]
	tmp			<- merge(tmp, tmp[, list(TOTAL_COMM_TYPE= sum(PART_EVER)), by='COMM_TYPE'], by='COMM_TYPE')
	tmp[, PART_EVER_P_COMM_TYPE:= PART_EVER/TOTAL_COMM_TYPE]
	tmp[, sum(PART_EVER)]
	tmp[INMIG_2YRS_LOC!='resident', sum(PART_EVER)]
	
	tmp2		<- desm[INMIG_2YRS_LOC!='resident', list(PART_EVER=sum(PART_EVER)), by=c('COMM_TYPE','INMIG_2YRS_LOC')]
	tmp2		<- merge(tmp2, tmp2[, list(TOTAL_COMM_TYPE= sum(PART_EVER)), by='COMM_TYPE'], by='COMM_TYPE')
	tmp2[, PART_EVER_P_COMM_TYPE:= PART_EVER/TOTAL_COMM_TYPE]
	
	tmp[, PART_EVER_P_COMM_TYPE:= PART_EVER/TOTAL_COMM_TYPE]
	
	
	tmp[, list(PART_EVER=sum(PART_EVER)), by='INMIG_2YRS_LOC']
	
	tmp[, list(MIG=sum(PART_EVER[INMIG_2YRS_LOC!='resident']), TOTAL_COMM_TYPE=sum(PART_EVER), MIG_P=sum(PART_EVER[INMIG_2YRS_LOC!='resident'])/sum(PART_EVER)), by='COMM_TYPE']
	
	dcast.data.table(tmp, COMM_TYPE~INMIG_2YRS_LOC, value.var='PART_EVER')
}

RakaiFull.phylogeography.190327.figure.migration.barcharts<- function()
{
	require(data.table)
	require(ggplot2)
	infile		<- '~/Dropbox (SPH Imperial College)/Rakai Fish Analysis/190327_sampling_by_gender_age.rda'
	...
	tmp			<- desm[, list(PART_EVER=sum(PART_EVER)), by=c('COMM_TYPE','INMIG_2YRS_LOC')]
	set(tmp, NULL, 'COMM_TYPE', tmp[, factor(COMM_TYPE, levels=c('inland','fisherfolk'), labels=c('inland communities','fishing sites'))])
	set(tmp, NULL, 'INMIG_2YRS_LOC', tmp[, factor(INMIG_2YRS_LOC, levels=c('resident','inland','fish','external','unknown'))])
	ggplot(tmp, aes(x=INMIG_2YRS_LOC, y=PART_EVER, fill=INMIG_2YRS_LOC)) +
			geom_bar(position=position_dodge(), stat='identity') +
			scale_y_sqrt(expand=c(0,0), lim=c(0,16000), breaks=c(100,1000,5e3,10e3)) +
			#scale_y_continuous(expand=c(0,0), lim=c(0,16000)) +
			scale_fill_manual(values=c('fish'='#7A5115','inland'='#66BD63','unknown'='grey70','resident'='darkorange1','external'='#00BFFF')) +
			facet_grid(~COMM_TYPE) +
			theme_bw() +
			...
			labs(x='', y='participants\n', fill='inmigration status')
	ggsave(file=paste0(outfile.base,'migration_barchart.pdf'),w=6,h=4)
	
	tmp			<- desm[, list(PART_EVER=sum(PART_EVER), HIV_1516_YES=sum(HIV_1516_YES)), by=c('COMM_TYPE','SEX','INMIG_2YRS_LOC')]
	tmp[, HIV_PREV:= HIV_1516_YES/PART_EVER]
	tmp			<- dcast.data.table(tmp, SEX+INMIG_2YRS_LOC~COMM_TYPE, value.var='HIV_PREV')
	set(tmp, NULL, 'SEX', tmp[, factor(SEX, levels=c('M','F'), labels=c('men','women'))])
	set(tmp, NULL, 'INMIG_2YRS_LOC', tmp[, factor(INMIG_2YRS_LOC, levels=c('resident','inland','fish','external','unknown'))])
	ggplot(tmp, aes(x=inland, y=fisherfolk, pch=SEX, colour=INMIG_2YRS_LOC)) +
			geom_abline(slope=1, lty=2) +
			geom_point(size=4) + 
			scale_x_continuous(labels=scales::percent, expand=c(0,0)) +
			scale_y_continuous(labels=scales::percent, expand=c(0,0)) +
			scale_colour_manual(values=c('fish'='#7A5115','inland'='#66BD63','unknown'='grey70','resident'='darkorange1','external'='#00BFFF')) +
			theme_bw() +
			coord_cartesian(xlim=c(0,0.75), ylim=c(0,0.75)) +
			labs(	x='\nHIV prevalence among immigrants to inland communities', 
					y='HIV prevalence among immigrants to fishing sites\n',
					colour='inmigration status',
					pch='gender')
	ggsave(file=paste0(outfile.base,'migration_prevalence_comparison.pdf'),w=5,h=4, useDingbats=FALSE)
}

RakaiFull.phylogeography.110327.data.eligibility.participation.sequenced<- function()
{
	require(data.table)
...
RakaiFull.phylogeography.190327.table.neff<- function()
{
	require(data.table)
	
	infile	<- '~/Dropbox (SPH Imperial College)/Rakai Fish Analysis/full_run/RakaiAll_output_190327_w250_s20_p25_d50_stagetwo_rerun23_min30_conf60_phylogeography_samcmc190327_nsweep1e5_opt112401_summary.csv'
	outfile.base<- gsub('\\.csv','',mcmc.file)
	df		<- as.data.table(read.csv(infile, stringsAsFactors=FALSE))
	df		<- subset(df, grepl('PI',VAR))
	df		<- df[order(NEFF)]
	
	ggplot(df, aes(x=NEFF)) + 
			geom_histogram(bins=40) + 
			theme_bw() +
			scale_y_continuous(expand=c(0,0), lim=c(0,22)) +
			scale_x_log10(breaks=c(100,300,500,1e3,2e3,5e3,10e3,15e3,30e3), lim=c(100,20e3),expand=c(0,0)) +
			labs(x='\neffective sample size')
	ggsave(file=paste0(outfile.base,'_neffhist.pdf'), w=10, h=6)  
}
...
RakaiFull.phylogeography.190327.flows.obs<- function()
{
	...
	indir				<- "~/Dropbox (SPH Imperial College)/Rakai Fish Analysis/full_run"	
	infile.inference	<- file.path(indir,"RakaiAll_output_190327_w250_s20_p25_d50_stagetwo_rerun23_min30_conf60_phylogeography_data_with_inmigrants.rda")
	...
	#	subset to variables needed, using RTR3
	rtr	<- subset(rtr3, select=c(	'PAIRID','TR_RID','TR_COMM_NUM','TR_COMM_NUM_A','TR_COMM_NUM_A','TR_COMM_NUM_A_MIG_2YR',
					'TR_SEX','TR_BIRTHDATE','TR_COMM_TYPE','TR_INMIG_LOC','TR_INMIGRATE_2YR',
					'REC_RID','REC_COMM_NUM','REC_COMM_NUM_A',
					'REC_SEX','REC_BIRTHDATE','REC_COMM_TYPE','REC_INMIGRATE_2YR'))
	...
	rtr[, list(TRM_OBS=length(PAIRID), P= round(100*length(PAIRID)/nrow(rtr),d=1)), by=c('TR_TRM_CATEGORY','REC_TRM_CATEGORY')]
...
RakaiFull.phylogeography.181006.flows.obs<- function()
{
	...
	# 	impact of migration on transmission dynamics
	z	<- copy(rtr3)
	z[, TRM_TYPE:='within_community']
	set(z, z[, which(grepl('mig',TR_COMM_NUM_A_MIG))], 'TRM_TYPE', 'inmigration')
	set(z, z[, which(!grepl('mig',TR_COMM_NUM_A_MIG) & TR_COMM_NUM_A_MIG!=REC_COMM_NUM_A)], 'TRM_TYPE', 'extracommunity')
	z[, table(TRM_TYPE)]
	z[, table(TRM_TYPE)/nrow(z)]
	
	#	impact of migration on cross-community transmission
	z[, TR_COMM_TYPE_MIG:= as.character(factor(substr(TR_COMM_NUM_A_MIG,1,1)%in%c('i','a','t'), level=c(TRUE,FALSE), label=c('inland','fisherfolk')))]
	set(z, z[, which(substr(TR_COMM_NUM_A_MIG,1,1)=='e')],'TR_COMM_TYPE_MIG','external_mig')
	set(z, z[, which(substr(TR_COMM_NUM_A_MIG,1,1)=='u')],'TR_COMM_TYPE_MIG','unknown_mig')
	z[, REC_COMM_TYPE:= as.character(factor(REC_COMM_TYPE=='fisherfolk', level=c(TRUE,FALSE), label=c('fisherfolk','inland')))]
	z	<- subset(z, TR_COMM_TYPE_MIG!=REC_COMM_TYPE & !TR_COMM_TYPE_MIG%in%c('unknown_mig','external_mig'))
	subset(z, select=c(TRM_TYPE, TR_COMM_TYPE_MIG, REC_COMM_TYPE))
	z[, table(TRM_TYPE)]
	z[, table(TRM_TYPE, TR_COMM_TYPE_MIG)]
